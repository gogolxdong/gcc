...
{
  return (CONVERT_EXPR_P(t) && TREE_TYPE(t) == integer_type_node && TREE_CODE(TREE_TYPE(TREE_OPERAND(t, 0))) == BOOLEAN_TYPE);
}
...
{
  if (VECTOR_TYPE_P(t1) && VECTOR_TYPE_P(t2) && (TYPE_VECTOR_OPAQUE(t1) || TYPE_VECTOR_OPAQUE(t2)) && tree_int_cst_equal(TYPE_SIZE(t1), TYPE_SIZE(t2)))
    return true;
...
bool vector_types_convertible_p(const_tree t1, const_tree t2, bool emit_lax_note)
{
  ...
  if ((TYPE_VECTOR_OPAQUE(t1) || TYPE_VECTOR_OPAQUE(t2)) && tree_int_cst_equal(TYPE_SIZE(t1), TYPE_SIZE(t2)))
    return true;

  convertible_lax =
      (tree_int_cst_equal(TYPE_SIZE(t1), TYPE_SIZE(t2)) && (TREE_CODE(TREE_TYPE(t1)) != REAL_TYPE || known_eq(TYPE_VECTOR_SUBPARTS(t1), TYPE_VECTOR_SUBPARTS(t2))) && (INTEGRAL_TYPE_P(TREE_TYPE(t1)) == INTEGRAL_TYPE_P(TREE_TYPE(t2))));

  if (!convertible_lax || flag_lax_vector_conversions)
    return convertible_lax;

  if (known_eq(TYPE_VECTOR_SUBPARTS(t1), TYPE_VECTOR_SUBPARTS(t2)) && lang_hooks.types_compatible_p(TREE_TYPE(t1), TREE_TYPE(t2)))
    return true;
...
/* Build a VEC_PERM_EXPR if V0, V1 and MASK are not error_mark_nodes
   ...
   the user.  */
tree c_build_vec_perm_expr(location_t loc, tree v0, tree v1, tree mask,
                           bool complain)
{
  tree ret;
  bool wrap = true;
  ...
  if (GET_MODE_BITSIZE(SCALAR_TYPE_MODE(TREE_TYPE(TREE_TYPE(v0)))) != GET_MODE_BITSIZE(SCALAR_TYPE_MODE(TREE_TYPE(TREE_TYPE(mask)))))
  {
    if (complain)
...
tree c_build_vec_convert(location_t loc1, tree expr, location_t loc2, tree type,
                         bool complain)
{
  ...
  if (!gnu_vector_type_p(TREE_TYPE(expr)) || (!VECTOR_INTEGER_TYPE_P(TREE_TYPE(expr)) && !VECTOR_FLOAT_TYPE_P(TREE_TYPE(expr))))
  {
    if (complain)
  ...
  if (!gnu_vector_type_p(type) || (!VECTOR_INTEGER_TYPE_P(type) && !VECTOR_FLOAT_TYPE_P(type)))
  {
    if (complain)
  ...
  if ((TYPE_MAIN_VARIANT(TREE_TYPE(TREE_TYPE(expr))) == TYPE_MAIN_VARIANT(TREE_TYPE(type))) || (VECTOR_INTEGER_TYPE_P(TREE_TYPE(expr)) && VECTOR_INTEGER_TYPE_P(type) && (TYPE_PRECISION(TREE_TYPE(TREE_TYPE(expr))) == TYPE_PRECISION(TREE_TYPE(type)))))
...
tree c_common_get_narrower(tree op, int *unsignedp_ptr)
{
  op = get_narrower(op, unsignedp_ptr);

  if (TREE_CODE(TREE_TYPE(op)) == ENUMERAL_TYPE && ENUM_IS_SCOPED(TREE_TYPE(op)))
  {
...
tree shorten_binary_op(tree result_type, tree op0, tree op1, bool bitwise)
{
  ...
  if ((TYPE_PRECISION(TREE_TYPE(op0)) == TYPE_PRECISION(TREE_TYPE(arg0))) && TREE_TYPE(op0) != result_type)
    unsigned0 = TYPE_UNSIGNED(TREE_TYPE(op0));
  if ((TYPE_PRECISION(TREE_TYPE(op1)) == TYPE_PRECISION(TREE_TYPE(arg1))) && TREE_TYPE(op1) != result_type)
  ...
  if ((TYPE_PRECISION(TREE_TYPE(arg0)) < TYPE_PRECISION(result_type)) && (TYPE_PRECISION(TREE_TYPE(arg1)) == TYPE_PRECISION(TREE_TYPE(arg0))) && unsigned0 == unsigned1 && (unsigned0 || !uns))
    return c_common_signed_or_unsigned_type(unsigned0, common_type(TREE_TYPE(arg0), TREE_TYPE(arg1)));

  else if (TREE_CODE(arg0) == INTEGER_CST && (unsigned1 || !uns) && (TYPE_PRECISION(TREE_TYPE(arg1)) < TYPE_PRECISION(result_type)) && (type = c_common_signed_or_unsigned_type(unsigned1, TREE_TYPE(arg1))) && !POINTER_TYPE_P(type) && int_fits_type_p(arg0, type))
    return type;

  else if (TREE_CODE(arg1) == INTEGER_CST && (unsigned0 || !uns) && (TYPE_PRECISION(TREE_TYPE(arg0)) < TYPE_PRECISION(result_type)) && (type = c_common_signed_or_unsigned_type(unsigned0, TREE_TYPE(arg0))) && !POINTER_TYPE_P(type) && int_fits_type_p(arg1, type))
    return type;
...
int_safely_convertible_to_real_p(const_tree from_type, const_tree to_type)
{
  ...
  return exact_real_truncate(TYPE_MODE(to_type), &real_low_bound) && exact_real_truncate(TYPE_MODE(to_type), &real_high_bound);
}
...
unsafe_conversion_p(tree type, tree expr, tree result, bool check_sign)
{
  ...
  if (TREE_CODE(expr) == REAL_CST || TREE_CODE(expr) == INTEGER_CST)
  {
    ...
    /* Warn for real constant that is not an exact integer converted
	 to integer type.  */
    if (TREE_CODE(expr_type) == REAL_TYPE && TREE_CODE(type) == INTEGER_TYPE)
    {
      if (!real_isinteger(TREE_REAL_CST_PTR(expr), TYPE_MODE(expr_type)))
        give_warning = UNSAFE_REAL;
    }
    /* Warn for an integer constant that does not fit into integer type.  */
    else if (TREE_CODE(expr_type) == INTEGER_TYPE && TREE_CODE(type) == INTEGER_TYPE && !int_fits_type_p(expr, type))
    {
      if (TYPE_UNSIGNED(type) && !TYPE_UNSIGNED(expr_type) && tree_int_cst_sgn(expr) < 0)
      {
        if (check_sign)
    ...
    else if (TREE_CODE(type) == REAL_TYPE)
    {
      ...
      /* Warn for a real constant that does not fit into a smaller
	     real type.  */
      else if (TREE_CODE(expr_type) == REAL_TYPE && TYPE_PRECISION(type) < TYPE_PRECISION(expr_type))
      {
  ...
  else if (TREE_CODE(expr) == COMPLEX_CST)
  {
    ...
    if ((TREE_CODE(imag_part) == REAL_CST && real_zerop(imag_part)) || (TREE_CODE(imag_part) == INTEGER_CST && integer_zerop(imag_part)))
  ...
  else
  {
    ...
    else if (TREE_CODE(expr_type) == INTEGER_TYPE && TREE_CODE(type) == INTEGER_TYPE)
    {
      ...
      if (TREE_CODE(expr) == BIT_AND_EXPR || TREE_CODE(expr) == BIT_IOR_EXPR || TREE_CODE(expr) == BIT_XOR_EXPR)
      {
        ...
        if (TREE_CODE(expr) == BIT_AND_EXPR)
        {
          ...
          if ((TREE_CODE(op0) == INTEGER_CST && int_fits_type_p(op0, c_common_signed_type(type)) && int_fits_type_p(op0, c_common_unsigned_type(type))) || (TREE_CODE(op1) == INTEGER_CST && int_fits_type_p(op1, c_common_signed_type(type)) && int_fits_type_p(op1, c_common_unsigned_type(type))))
            return SAFE_CONVERSION;
          /* If constant is unsigned and fits in the target
		     type, then the result will also fit.  */
          else if ((TREE_CODE(op0) == INTEGER_CST && unsigned0 && int_fits_type_p(op0, type)) || (TREE_CODE(op1) == INTEGER_CST && unsigned1 && int_fits_type_p(op1, type)))
            return SAFE_CONVERSION;
        }
      }
      ...
      else if (((TYPE_PRECISION(type) == TYPE_PRECISION(expr_type) && TYPE_UNSIGNED(expr_type) != TYPE_UNSIGNED(type))
    ...
    else if (TREE_CODE(expr_type) == INTEGER_TYPE && TREE_CODE(type) == REAL_TYPE)
    {
    ...
    else if (TREE_CODE(expr_type) == REAL_TYPE && TREE_CODE(type) == REAL_TYPE && TYPE_PRECISION(type) < TYPE_PRECISION(expr_type))
      give_warning = UNSAFE_REAL;

    /* Check conversion between two complex types.  */
    else if (TREE_CODE(expr_type) == COMPLEX_TYPE && TREE_CODE(type) == COMPLEX_TYPE)
    {
      ...
      /* Warn for real types converted to integer types.  */
      if (TREE_CODE(from_type) == REAL_TYPE && TREE_CODE(to_type) == INTEGER_TYPE)
        give_warning = UNSAFE_REAL;

      /* Warn for real types converted to smaller real types.  */
      else if (TREE_CODE(from_type) == REAL_TYPE && TREE_CODE(to_type) == REAL_TYPE && TYPE_PRECISION(to_type) < TYPE_PRECISION(from_type))
        give_warning = UNSAFE_REAL;
      ...
      else if (TREE_CODE(from_type) == INTEGER_TYPE && TREE_CODE(to_type) == INTEGER_TYPE)
      {
        /* Warn for integer types converted to smaller integer types.  */
        ...
        else if (((TYPE_PRECISION(to_type) == TYPE_PRECISION(from_type) && TYPE_UNSIGNED(to_type) != TYPE_UNSIGNED(from_type)) || (TYPE_UNSIGNED(to_type) && !TYPE_UNSIGNED(from_type))) && check_sign)
          give_warning = UNSAFE_SIGN;
      }
      else if (TREE_CODE(from_type) == INTEGER_TYPE && TREE_CODE(to_type) == REAL_TYPE && !int_safely_convertible_to_real_p(from_type, to_type))
        give_warning = UNSAFE_OTHER;
    }
...
tree convert_and_check(location_t loc, tree type, tree expr)
{
  tree result;
                      ...
                      int only_writes)
{
  ...
  while (list)
  {
    if (candidate_equal_p(list->expr, written) && !candidate_equal_p(list->writer, writer) && (!only_writes || list->writer))
    {
...
tree c_common_fixed_point_type_for_size(unsigned int ibit, unsigned int fbit,
...
tree c_common_type_for_mode(machine_mode mode, int unsignedp)
{
  tree t;
  int i;
  ...
  for (i = 0; i < NUM_FLOATN_NX_TYPES; i++)
    if (FLOATN_NX_TYPE_NODE(i) != NULL_TREE && mode == TYPE_MODE(FLOATN_NX_TYPE_NODE(i)))
      return FLOATN_NX_TYPE_NODE(i);

  if (mode == TYPE_MODE(void_type_node))
    return void_type_node;

  if (mode == TYPE_MODE(build_pointer_type(char_type_node)) || mode == TYPE_MODE(build_pointer_type(integer_type_node)))
  {
    unsigned int precision = GET_MODE_PRECISION(as_a<scalar_int_mode>(mode));
    return (unsignedp
  ...
  if (COMPLEX_MODE_P(mode))
  {
    ...
    for (i = 0; i < NUM_FLOATN_NX_TYPES; i++)
      if (COMPLEX_FLOATN_NX_TYPE_NODE(i) != NULL_TREE && mode == TYPE_MODE(COMPLEX_FLOATN_NX_TYPE_NODE(i)))
  ...
  else if (GET_MODE_CLASS(mode) == MODE_VECTOR_BOOL && valid_vector_subparts_p(GET_MODE_NUNITS(mode)))
  {
  ...
  if (dfloat128_type_node != NULL_TREE && mode == TYPE_MODE(dfloat128_type_node))
  ...
  for (t = registered_builtin_types; t; t = TREE_CHAIN(t))
  {
    ...
    if (TYPE_MODE(type) == mode && VECTOR_TYPE_P(type) == VECTOR_MODE_P(mode) && !!unsignedp == !!TYPE_UNSIGNED(type))
      return type;
  }
  return NULL_TREE;
}
...
tree c_common_signed_or_unsigned_type(int unsignedp, tree type)
{
  ...
  for (i = 0; i < NUM_INT_N_ENTS; i++)
    if (int_n_enabled_p[i] && (type1 == int_n_trees[i].unsigned_type || type1 == int_n_trees[i].signed_type))
      return (unsignedp ? int_n_trees[i].unsigned_type
                        : int_n_trees[i].signed_type);
...
#define C_COMMON_FIXED_TYPES(NAME)                                                             \
  if (type1 == short_##NAME##_type_node || type1 == unsigned_short_##NAME##_type_node)         \
    return unsignedp ? unsigned_short_##NAME##_type_node                                       \
                     : short_##NAME##_type_node;                                               \
  if (type1 == NAME##_type_node || type1 == unsigned_##NAME##_type_node)                       \
    return unsignedp ? unsigned_##NAME##_type_node                                             \
                     : NAME##_type_node;                                                       \
  if (type1 == long_##NAME##_type_node || type1 == unsigned_long_##NAME##_type_node)           \
    return unsignedp ? unsigned_long_##NAME##_type_node                                        \
                     : long_##NAME##_type_node;                                                \
  if (type1 == long_long_##NAME##_type_node || type1 == unsigned_long_long_##NAME##_type_node) \
    return unsignedp ? unsigned_long_long_##NAME##_type_node                                   \
                     : long_long_##NAME##_type_node;

#define C_COMMON_FIXED_MODE_TYPES(NAME)                          \
  if (type1 == NAME##_type_node || type1 == u##NAME##_type_node) \
    return unsignedp ? u##NAME##_type_node                       \
                     : NAME##_type_node;

#define C_COMMON_FIXED_TYPES_SAT(NAME)                                                                     \
  if (type1 == sat_##short_##NAME##_type_node || type1 == sat_##unsigned_short_##NAME##_type_node)         \
    return unsignedp ? sat_##unsigned_short_##NAME##_type_node                                             \
                     : sat_##short_##NAME##_type_node;                                                     \
  if (type1 == sat_##NAME##_type_node || type1 == sat_##unsigned_##NAME##_type_node)                       \
    return unsignedp ? sat_##unsigned_##NAME##_type_node                                                   \
                     : sat_##NAME##_type_node;                                                             \
  if (type1 == sat_##long_##NAME##_type_node || type1 == sat_##unsigned_long_##NAME##_type_node)           \
    return unsignedp ? sat_##unsigned_long_##NAME##_type_node                                              \
                     : sat_##long_##NAME##_type_node;                                                      \
  if (type1 == sat_##long_long_##NAME##_type_node || type1 == sat_##unsigned_long_long_##NAME##_type_node) \
    return unsignedp ? sat_##unsigned_long_long_##NAME##_type_node                                         \
                     : sat_##long_long_##NAME##_type_node;

#define C_COMMON_FIXED_MODE_TYPES_SAT(NAME)                                  \
  if (type1 == sat_##NAME##_type_node || type1 == sat_##u##NAME##_type_node) \
...
#define TYPE_OK(node) \
  (TYPE_MODE(type) == TYPE_MODE(node) && TYPE_PRECISION(type) == TYPE_PRECISION(node))
  ...
  for (i = 0; i < NUM_INT_N_ENTS; i++)
    if (int_n_enabled_p[i] && TYPE_MODE(type) == int_n_data[i].m && TYPE_PRECISION(type) == int_n_data[i].bitsize)
      return (unsignedp ? int_n_trees[i].unsigned_type
                        : int_n_trees[i].signed_type);
...
void binary_op_error(rich_location *richloc, enum tree_code code,
...
tree shorten_compare(location_t loc, tree *op0_ptr, tree *op1_ptr,
                     tree *restype_ptr, enum tree_code *rescode_ptr)
{
  ...
  if (TYPE_PRECISION(TREE_TYPE(primop0)) < TYPE_PRECISION(TREE_TYPE(op0)) && TYPE_PRECISION(TREE_TYPE(op0)) < TYPE_PRECISION(*restype_ptr) && !unsignedp0 && TYPE_UNSIGNED(TREE_TYPE(op0)))
    primop0 = op0;
  if (TYPE_PRECISION(TREE_TYPE(primop1)) < TYPE_PRECISION(TREE_TYPE(op1)) && TYPE_PRECISION(TREE_TYPE(op1)) < TYPE_PRECISION(*restype_ptr) && !unsignedp1 && TYPE_UNSIGNED(TREE_TYPE(op1)))
    primop1 = op1;
  ...
  if (TREE_CONSTANT(primop0) && !integer_zerop(primop1) && !real_zerop(primop1) && !fixed_zerop(primop1))
  {
  ...
  if (!real1 && !real2 && TREE_CODE(TREE_TYPE(primop0)) != FIXED_POINT_TYPE && TREE_CODE(primop1) == INTEGER_CST && TYPE_PRECISION(TREE_TYPE(primop0)) < TYPE_PRECISION(*restype_ptr))
  {
    ...
    if (TREE_CODE(primop0) != INTEGER_CST
        ...
        && !(from_macro_expansion_at(expansion_point_location_if_in_system_header(EXPR_LOCATION(primop0)))))
    {
  ...
  else if (real1 && real2 && DECIMAL_FLOAT_MODE_P(TYPE_MODE(TREE_TYPE(primop0))) && DECIMAL_FLOAT_MODE_P(TYPE_MODE(TREE_TYPE(primop1))))
  ...
  else if (real1 && real2 && (TYPE_PRECISION(TREE_TYPE(primop0)) == TYPE_PRECISION(TREE_TYPE(primop1))))
  ...
  else if (unsignedp0 == unsignedp1 && real1 == real2 && TYPE_PRECISION(TREE_TYPE(primop0)) < TYPE_PRECISION(*restype_ptr) && TYPE_PRECISION(TREE_TYPE(primop1)) < TYPE_PRECISION(*restype_ptr))
  {
    type = common_type(TREE_TYPE(primop0), TREE_TYPE(primop1));
    type = c_common_signed_or_unsigned_type(unsignedp0 || TYPE_UNSIGNED(*restype_ptr),
                                            type);
    /* Make sure shorter operand is extended the right way
	 to match the longer operand.  */
    primop0 = convert(c_common_signed_or_unsigned_type(unsignedp0,
                                                       TREE_TYPE(primop0)),
                      primop0);
    primop1 = convert(c_common_signed_or_unsigned_type(unsignedp1,
                                                       TREE_TYPE(primop1)),
                      primop1);
  }
  else
  {
    ...
    if (!real1 && !real2 && integer_zerop(primop1) && TYPE_UNSIGNED(*restype_ptr))
    {
      ...
      bool warn =
          warn_type_limits && !in_system_header_at(loc) && !(TREE_CODE(folded_op0) == INTEGER_CST && !TREE_OVERFLOW(convert(c_common_signed_type(type), folded_op0)))
...
tree pointer_int_sum(location_t loc, enum tree_code resultcode,
                     tree ptrop, tree intop, bool complain)
{
  ...
  if ((TREE_CODE(intop) == PLUS_EXPR || TREE_CODE(intop) == MINUS_EXPR) && !TREE_CONSTANT(intop) && TREE_CONSTANT(TREE_OPERAND(intop, 1)) && TREE_CONSTANT(size_exp)
	 ...
	 an overflow error if the constant is negative but INTOP is not.  */
      && (!TYPE_UNSIGNED(TREE_TYPE(intop)) || (TYPE_PRECISION(TREE_TYPE(intop)) == TYPE_PRECISION(TREE_TYPE(ptrop)))))
  {
  ...
  if (TYPE_PRECISION(TREE_TYPE(intop)) != TYPE_PRECISION(sizetype) || TYPE_UNSIGNED(TREE_TYPE(intop)) != TYPE_UNSIGNED(sizetype))
...
tree c_wrap_maybe_const(tree expr, bool non_const)
{
...
bool decl_with_nonnull_addr_p(const_tree expr)
{
  return (DECL_P(expr) && (TREE_CODE(expr) == PARM_DECL || TREE_CODE(expr) == LABEL_DECL || !DECL_WEAK(expr)));
}
...
tree c_common_truthvalue_conversion(location_t location, tree expr)
{
  ...
  case INTEGER_CST:
    if (TREE_CODE(TREE_TYPE(expr)) == ENUMERAL_TYPE && !integer_zerop(expr) && !integer_onep(expr))
  ...
  case RROTATE_EXPR:
    ...
    if (TREE_SIDE_EFFECTS(TREE_OPERAND(expr, 1)))
    {
      expr = build2(COMPOUND_EXPR, truthvalue_type_node,
                    TREE_OPERAND(expr, 1),
                    c_common_truthvalue_conversion(location, TREE_OPERAND(expr, 0)));
      goto ret;
    }
    else
  ...
  case LSHIFT_EXPR:
    ...
    if (TREE_CODE(TREE_TYPE(expr)) == INTEGER_TYPE && !TYPE_UNSIGNED(TREE_TYPE(expr)))
      warning_at(EXPR_LOCATION(expr), OPT_Wint_in_bool_context,
                 "%<<<%> in boolean context, did you mean %<<%>?");
    break;

  case COND_EXPR:
    if (warn_int_in_bool_context && !from_macro_definition_at(EXPR_LOCATION(expr)))
    {
      tree val1 = fold_for_warn(TREE_OPERAND(expr, 1));
      tree val2 = fold_for_warn(TREE_OPERAND(expr, 2));
      if (TREE_CODE(val1) == INTEGER_CST && TREE_CODE(val2) == INTEGER_CST && !integer_zerop(val1) && !integer_zerop(val2) && (!integer_onep(val1) || !integer_onep(val2)))
        warning_at(EXPR_LOCATION(expr), OPT_Wint_in_bool_context,
                   "%<?:%> using integer constants in boolean context, "
                   "the expression will always evaluate to %<true%>");
      else if ((TREE_CODE(val1) == INTEGER_CST && !integer_zerop(val1) && !integer_onep(val1)) || (TREE_CODE(val2) == INTEGER_CST && !integer_zerop(val2) && !integer_onep(val2)))
        warning_at(EXPR_LOCATION(expr), OPT_Wint_in_bool_context,
                   "%<?:%> using integer constants in boolean context");
    }
  ...
  CASE_CONVERT:
  {
    ...
    if (POINTER_TYPE_P(totype) && !c_inhibit_evaluation_warnings && TREE_CODE(fromtype) == REFERENCE_TYPE)
    {
      tree inner = expr;
      STRIP_NOPS(inner);
    ...
    if (TREE_CODE(totype) == REFERENCE_TYPE || TREE_CODE(fromtype) == REFERENCE_TYPE)
      break;
    /* Don't strip a conversion from C++0x scoped enum, since they
	   don't implicitly convert to other types.  */
    if (TREE_CODE(fromtype) == ENUMERAL_TYPE && ENUM_IS_SCOPED(fromtype))
      break;
  ...
  case MODIFY_EXPR:
    if (!TREE_NO_WARNING(expr) && warn_parentheses && warning_at(location, OPT_Wparentheses, "suggest parentheses around assignment used as "
                                                                                             "truth value"))
      TREE_NO_WARNING(expr) = 1;
    break;
...
void c_apply_type_quals_to_decl(int type_quals, tree decl)
{
  ...
  if ((type_quals & TYPE_QUAL_CONST) || (type && TREE_CODE(type) == REFERENCE_TYPE))
  ...
  if (type_quals & TYPE_QUAL_RESTRICT)
  {
    while (type && TREE_CODE(type) == ARRAY_TYPE)
      ...
      type = TREE_TYPE(type);
    if (!type || !POINTER_TYPE_P(type) || !C_TYPE_OBJECT_OR_INCOMPLETE_P(TREE_TYPE(type)))
...
tree c_sizeof_or_alignof_type(location_t loc,
                              ...
                              int complain)
{
  ...
  {
    if (type_code == VOID_TYPE && complain && warn_pointer_arith)
      pedwarn(loc, OPT_Wpointer_arith,
  ...
  else if (!COMPLETE_TYPE_P(type) && (!c_dialect_cxx() || is_sizeof || type_code != ARRAY_TYPE))
  {
    if (complain)
  ...
  else if (c_dialect_cxx() && type_code == ARRAY_TYPE && !COMPLETE_TYPE_P(TREE_TYPE(type)))
  {
    if (complain)
...
tree c_alignof_expr(location_t loc, tree expr)
{
  tree t;
  ...
  else if (TREE_CODE(expr) == COMPONENT_REF && DECL_C_BIT_FIELD(TREE_OPERAND(expr, 1)))
  {
  ...
  else if (INDIRECT_REF_P(expr))
  {
    ...
    while (CONVERT_EXPR_P(t) && TREE_CODE(TREE_TYPE(TREE_OPERAND(t, 0))) == POINTER_TYPE)
    {
      int thisalign;
...
void c_common_nodes_and_builtins(void)
{
  ...
  if (targetm.scalar_mode_supported_p(SDmode) && targetm.scalar_mode_supported_p(DDmode) && targetm.scalar_mode_supported_p(TDmode))
  {
  ...
  for (unsigned i = 0;
       ...
       ++i)
    builtin_structptr_types[i].node = build_variant_type_copy(builtin_structptr_types[i].base);
     ...
     record_builtin_type gave names to the built-in types. */
  {
    ...
    TYPE_NAME(build_qualified_type(void_type_node, TYPE_QUAL_CONST)) = void_name;
  ...
  char8_array_type_node = build_array_type(char8_type_node, array_domain_type);
  ...
  char16_array_type_node = build_array_type(char16_type_node, array_domain_type);
  ...
  char32_array_type_node = build_array_type(char32_type_node, array_domain_type);
  ...
  default_function_type = build_varargs_function_type_list(integer_type_node, NULL_TREE);
...
tree build_va_arg(location_t loc, tree expr, tree type)
{
  ...
  if (va_type == error_mark_node || canon_va_type == NULL_TREE)
  {
  ...
  if (TREE_CODE(va_type) == ARRAY_TYPE)
  {
    ...
    tree canon_expr_type = targetm.canonical_va_list_type(TREE_TYPE(expr));
  ...
  else
  {
    ...
    gcc_assert(TYPE_MAIN_VARIANT(TREE_TYPE(va_type)) == TYPE_MAIN_VARIANT(TREE_TYPE(canon_va_type)));
              ...
              tree fnattrs, bool implicit_p)
{
  ...
  gcc_assert((!both_p && !fallback_p) || !strncmp(name, "__builtin_",
                                                  strlen("__builtin_")));
  ...
  if (both_p && !flag_no_builtin && !builtin_function_disabled_p(libname) && !(nonansi_p && flag_no_nonansi_builtin))
...
bool c_promoting_integer_type_p(const_tree t)
{
  switch (TREE_CODE(t))
  {
  case INTEGER_TYPE:
    return (TYPE_MAIN_VARIANT(t) == char_type_node || TYPE_MAIN_VARIANT(t) == signed_char_type_node || TYPE_MAIN_VARIANT(t) == unsigned_char_type_node || TYPE_MAIN_VARIANT(t) == short_integer_type_node || TYPE_MAIN_VARIANT(t) == short_unsigned_type_node || TYPE_PRECISION(t) < TYPE_PRECISION(integer_type_node));
...
bool self_promoting_args_p(const_tree parms)
{
  const_tree t;
...
tree strip_pointer_or_array_types(tree t)
{
...
tree c_add_case_label(location_t loc, splay_tree cases, tree cond,
                      tree low_value, tree high_value)
{
  ...
  if ((low_value && TREE_TYPE(low_value) && POINTER_TYPE_P(TREE_TYPE(low_value))) || (high_value && TREE_TYPE(high_value) && POINTER_TYPE_P(TREE_TYPE(high_value))))
  {
  ...
  if (!node && (low_value || high_value))
  {
    ...
    if (low_bound && CASE_HIGH((tree)low_bound->value) && tree_int_cst_compare(CASE_HIGH((tree)low_bound->value), low_value) >= 0)
      node = low_bound;
    /* Check to see if the HIGH_BOUND overlaps.  The low end of that
	 range is bigger than the low end of the current range, so we
	 are only interested if the current range is a real range, and
	 not an ordinary case label.  */
    else if (high_bound && high_value && (tree_int_cst_compare((tree)high_bound->key, high_value) <= 0))
      node = high_bound;
  }
...
c_switch_covers_all_cases_p_1(splay_tree_node node, void *data)
{
  ...
  else if (wi::add(wi::to_widest(args[0]), 1) != wi::to_widest(CASE_LOW(label)))
    return 1;
  if (CASE_HIGH(label))
...
bool c_switch_covers_all_cases_p(splay_tree cases, tree type)
{
  /* If there is default:, this is always the case.  */
  splay_tree_node default_node = splay_tree_lookup(cases, (splay_tree_key)NULL);
  if (default_node)
    return true;
  ...
  if (args[0] == NULL_TREE || wi::to_widest(args[0]) < wi::to_widest(TYPE_MAX_VALUE(type)))
    return false;
...
tree finish_label_address_expr(tree label, location_t loc)
{
  tree result;
...
#define DEF_ATTR_TREE_LIST(ENUM, PURPOSE, VALUE, CHAIN)                         \
  built_in_attributes[(int)ENUM] = tree_cons(built_in_attributes[(int)PURPOSE], \
...
int check_user_alignment(const_tree align, bool objfile, bool warn_zero)
{
  if (error_operand_p(align))
    return -1;

  if (TREE_CODE(align) != INTEGER_CST || !INTEGRAL_TYPE_P(TREE_TYPE(align)))
  {
  ...
  int log2align;
  if (tree_int_cst_sgn(align) == -1 || (log2align = tree_log2(align)) == -1)
  {
...
bool c_determine_visibility(tree decl)
{
  ...
  if (lookup_attribute("visibility", DECL_ATTRIBUTES(decl)) || (TARGET_DLLIMPORT_DECL_ATTRIBUTES && (lookup_attribute("dllimport", DECL_ATTRIBUTES(decl)) || lookup_attribute("dllexport", DECL_ATTRIBUTES(decl)))))
    return true;
  ...
  {
    if (visibility_options.inpragma || DECL_VISIBILITY(decl) != default_visibility)
    {
      DECL_VISIBILITY(decl) = default_visibility;
      DECL_VISIBILITY_SPECIFIED(decl) = visibility_options.inpragma;
      /* If visibility changed and DECL already has DECL_RTL, ensure
	     symbol flags are updated.  */
      if (((VAR_P(decl) && TREE_STATIC(decl)) || TREE_CODE(decl) == FUNCTION_DECL) && DECL_RTL_SET_P(decl))
        make_decl_rtl(decl);
    }
  }
  return false;
...
check_function_sentinel(const_tree fntype, int nargs, tree *argarray)
{
  ...
  if (attr)
  {
    ...
    if ((!POINTER_TYPE_P(TREE_TYPE(sentinel)) || !integer_zerop(sentinel))
                        ...
                        int nargs, tree *unfolded_argarray)
{
  ...
  if (fndecl && TREE_CODE(fndecl) == FUNCTION_DECL)
  {
    ...
    if (fndecl_built_in_p(fndecl, BUILT_IN_NORMAL) && nargs == 3 && TREE_CODE(argarray[2]) == INTEGER_CST && integer_zerop(argarray[2]))
      return false;
  ...
  for (i = 0; i < nargs && parms && parms != void_list_node; i++)
  {
    ...
    if (POINTER_TYPE_P(type) && TYPE_RESTRICT(type) && !TYPE_READONLY(TREE_TYPE(type)))
...
bool get_attribute_operand(tree arg_num_expr, unsigned HOST_WIDE_INT *valp)
{
...
bool parse_optimize_options(tree args, bool attr_p)
{
  bool ret = true;
  unsigned opt_argc;
  unsigned i;
...
bool attribute_fallthrough_p(tree attr)
{
...
bool check_function_arguments(location_t loc, const_tree fndecl, const_tree fntype,
...
void check_function_arguments_recurse(void (*callback)(void *, tree, unsigned HOST_WIDE_INT),
                                      void *ctx, tree param,
                                      unsigned HOST_WIDE_INT param_num)
{
  if (CONVERT_EXPR_P(param) && (TYPE_PRECISION(TREE_TYPE(param)) == TYPE_PRECISION(TREE_TYPE(TREE_OPERAND(param, 0)))))
  {
    /* Strip coercion.  */
...
bool check_builtin_function_arguments(location_t loc, vec<location_t> arg_loc,
                                      ...
                                      int nargs, tree *args)
{
  ...
  if (DECL_BUILT_IN_CLASS(fndecl) == BUILT_IN_MD)
    return (!targetm.check_builtin_call || targetm.check_builtin_call(loc, arg_loc, fndecl,
  ...
  case BUILT_IN_ALLOCA_WITH_ALIGN:
  {
    /* Get the requested alignment (in bits) if it's a constant
	   integer expression.  */
    unsigned HOST_WIDE_INT align = tree_fits_uhwi_p(args[1]) ? tree_to_uhwi(args[1]) : 0;
  ...
  case BUILT_IN_ISUNORDERED:
    if (builtin_function_validate_nargs(loc, fndecl, nargs, 2))
    {
      ...
      if (!((code0 == REAL_TYPE && code1 == REAL_TYPE) || (code0 == REAL_TYPE && code1 == INTEGER_TYPE) || (code0 == INTEGER_TYPE && code1 == REAL_TYPE)))
      {
  ...
  case BUILT_IN_MUL_OVERFLOW:
    if (builtin_function_validate_nargs(loc, fndecl, nargs, 3))
    {
      ...
      if (TREE_CODE(TREE_TYPE(args[2])) != POINTER_TYPE || !INTEGRAL_TYPE_P(TREE_TYPE(TREE_TYPE(args[2]))))
      {
...
void c_parse_error(const char *gmsgid, enum cpp_ttype token_type,
...
#define catenate_messages(M1, M2) catenate_strings((M1), (M2), sizeof(M2))
  ...
  else if (token_type == CPP_CHAR || token_type == CPP_WCHAR || token_type == CPP_CHAR16 || token_type == CPP_CHAR32 || token_type == CPP_UTF8CHAR)
  {
  ...
  else if (token_type == CPP_CHAR_USERDEF || token_type == CPP_WCHAR_USERDEF || token_type == CPP_CHAR16_USERDEF || token_type == CPP_CHAR32_USERDEF || token_type == CPP_UTF8CHAR_USERDEF)
    message = catenate_messages(gmsgid,
                                " before user-defined character literal");
  else if (token_type == CPP_STRING_USERDEF || token_type == CPP_WSTRING_USERDEF || token_type == CPP_STRING16_USERDEF || token_type == CPP_STRING32_USERDEF || token_type == CPP_UTF8STRING_USERDEF)
    message = catenate_messages(gmsgid, " before user-defined string literal");
  else if (token_type == CPP_STRING || token_type == CPP_WSTRING || token_type == CPP_STRING16 || token_type == CPP_STRING32 || token_type == CPP_UTF8STRING)
...
bool c_cpp_diagnostic(cpp_reader *pfile ATTRIBUTE_UNUSED,
...
c_common_to_target_charset(HOST_WIDE_INT c)
{
  ...
  if (flag_signed_char)
    return ((HOST_WIDE_INT)uc) << (HOST_BITS_PER_WIDE_INT - CHAR_TYPE_SIZE) >> (HOST_BITS_PER_WIDE_INT - CHAR_TYPE_SIZE);
  else
    return uc;
}
...
tree fold_offsetof(tree expr, tree type, enum tree_code ctx)
{
  tree base, off, t;
  ...
  case COMPONENT_REF:
    ...
    off = size_binop_loc(input_location, PLUS_EXPR, DECL_FIELD_OFFSET(t),
                         size_int(tree_to_uhwi(DECL_FIELD_BIT_OFFSET(t)) / BITS_PER_UNIT));
    break;
  ...
  case ARRAY_REF:
    ...
    if (TREE_CODE(t) == INTEGER_CST && tree_int_cst_sgn(t) >= 0)
    {
      ...
      if (upbound != NULL_TREE && TREE_CODE(upbound) == INTEGER_CST && !tree_int_cst_equal(upbound, TYPE_MAX_VALUE(TREE_TYPE(upbound))))
      {
        ...
        if (tree_int_cst_lt(upbound, t))
        {
          ...
          for (v = TREE_OPERAND(expr, 0);
               ...
               v = TREE_OPERAND(v, 0))
            if (TREE_CODE(TREE_TYPE(TREE_OPERAND(v, 0))) == RECORD_TYPE)
            {
          ...
          /* Don't warn if the array might be considered a poor
		     ...
		     definition thereof.  */
          if (TREE_CODE(v) == ARRAY_REF || TREE_CODE(v) == COMPONENT_REF)
...
int complete_array_type(tree *ptype, tree initial_value, bool do_default)
{
  ...
  TYPE_DOMAIN(main_type) = build_range_type(TREE_TYPE(maxindex),
  ...
  if (TYPE_STRUCTURAL_EQUALITY_P(TREE_TYPE(main_type)) || TYPE_STRUCTURAL_EQUALITY_P(TYPE_DOMAIN(main_type)))
    SET_TYPE_STRUCTURAL_EQUALITY(main_type);
  else if (TYPE_CANONICAL(TREE_TYPE(main_type)) != TREE_TYPE(main_type) || (TYPE_CANONICAL(TYPE_DOMAIN(main_type)) != TYPE_DOMAIN(main_type)))
    TYPE_CANONICAL(main_type) = build_array_type(TYPE_CANONICAL(TREE_TYPE(main_type)),
  ...
  if (COMPLETE_TYPE_P(type) && TREE_CODE(TYPE_SIZE_UNIT(type)) == INTEGER_CST && (overflow_p || TREE_OVERFLOW(TYPE_SIZE_UNIT(type))))
  {
...
void complete_flexible_array_elts(tree init)
{
  tree elt, type;
  ...
  type = TREE_TYPE(elt);
  if (TREE_CODE(type) == ARRAY_TYPE && TYPE_SIZE(type) == NULL_TREE)
...
void c_common_mark_addressable_vec(tree t)
{
  if (TREE_CODE(t) == C_MAYBE_CONST_EXPR)
    t = C_MAYBE_CONST_EXPR_EXPR(t);
  while (handled_component_p(t))
    t = TREE_OPERAND(t, 0);
  if (!VAR_P(t) && TREE_CODE(t) != PARM_DECL && TREE_CODE(t) != COMPOUND_LITERAL_EXPR)
    return;
...
speculation_safe_value_resolve_call(tree function, vec<tree, va_gc> *params)
{
  ...
  if (size == 1 || size == 2 || size == 4 || size == 8 || size == 16)
    return ((enum built_in_function)((int)BUILT_IN_SPECULATION_SAFE_VALUE_1 + exact_log2(size)));
                                      ...
                                      vec<tree, va_gc> *params)
{
  ...
  if (!(TREE_CODE(TREE_TYPE(val)) == POINTER_TYPE || TREE_CODE(TREE_TYPE(val)) == INTEGER_TYPE))
  {
    error_at(loc,
  ...
  if (params->length() == 2)
  {
    ...
    if (!(TREE_TYPE(val) == TREE_TYPE(val2) || useless_type_conversion_p(TREE_TYPE(val), TREE_TYPE(val2))))
    {
                        ...
                        vec<tree, va_gc> *params)
{
  ...
  for (x = 0; x < n_param - n_model; x++)
  {
    ...
    else if (TYPE_SIZE_UNIT(TREE_TYPE(type)) && TREE_CODE((TYPE_SIZE_UNIT(TREE_TYPE(type)))) != INTEGER_CST)
    {
...
tree resolve_overloaded_builtin(location_t loc, tree function,
                                vec<tree, va_gc> *params)
{
  ...
  case BUILT_IN_SPECULATION_SAFE_VALUE_N:
  {
    ...
    enum built_in_function fncode = speculation_safe_value_resolve_call(function, params);
    ;
    ...
    else
    {
      ...
      /* If the optional second argument is present, handle any side
	       effects now.  */
      if (params->length() == 2 && TREE_SIDE_EFFECTS((*params)[1]))
  ...
  case BUILT_IN_SYNC_LOCK_RELEASE_N:
  {
    ...
    if (fetch_op)
      fetch_op =
          (orig_code != BUILT_IN_SYNC_BOOL_COMPARE_AND_SWAP_N && orig_code != BUILT_IN_SYNC_VAL_COMPARE_AND_SWAP_N && orig_code != BUILT_IN_SYNC_LOCK_TEST_AND_SET_N && orig_code != BUILT_IN_SYNC_LOCK_RELEASE_N);
    ...
    if (result == error_mark_node)
      return result;
    if (orig_code != BUILT_IN_SYNC_BOOL_COMPARE_AND_SWAP_N && orig_code != BUILT_IN_SYNC_LOCK_RELEASE_N && orig_code != BUILT_IN_ATOMIC_STORE_N && orig_code != BUILT_IN_ATOMIC_COMPARE_EXCHANGE_N)
...
bool vector_types_compatible_elements_p(tree t1, tree t2)
{
  ...
  gcc_assert((INTEGRAL_TYPE_P(t1) || c1 == REAL_TYPE || c1 == FIXED_POINT_TYPE) && (INTEGRAL_TYPE_P(t2) || c2 == REAL_TYPE || c2 == FIXED_POINT_TYPE));
  ...
  if (t1 == t2)
    return true;
  if (opaque && c1 == c2 && (INTEGRAL_TYPE_P(t1) || c1 == REAL_TYPE) && TYPE_PRECISION(t1) == TYPE_PRECISION(t2))
    return true;
  return false;
}
...
void set_underlying_type(tree x)
{
...
bool user_facing_original_type_p(const_tree type)
{
...
void record_types_used_by_current_var_decl(tree decl)
{
...
void release_tree_vector(vec<tree, va_gc> *vec)
{
  if (vec != NULL)
  {
    vec->truncate(0);
...
bool keyword_begins_type_specifier(enum rid keyword)
{
  switch (keyword)
  {
  case RID_AUTO_TYPE:
...
bool keyword_is_type_qualifier(enum rid keyword)
{
  switch (keyword)
  {
  case RID_CONST:
...
bool keyword_is_storage_class_specifier(enum rid keyword)
{
  switch (keyword)
  {
  case RID_STATIC:
...
bool keyword_is_decl_specifier(enum rid keyword)
{
  if (keyword_is_storage_class_specifier(keyword) || keyword_is_type_qualifier(keyword) || keyword_is_function_specifier(keyword))
    return true;
...
tree build_userdef_literal(tree suffix_id, tree value,
...
bool convert_vector_to_array_for_subscript(location_t loc,
                                           tree *vecp, tree index)
{
  ...
  if (gnu_vector_type_p(TREE_TYPE(*vecp)))
  {
    ...
    if (TREE_CODE(index) == INTEGER_CST)
      if (!tree_fits_uhwi_p(index) || maybe_ge(tree_to_uhwi(index), TYPE_VECTOR_SUBPARTS(type)))
                 ...
                 bool complain)
{
  ...
  case RSHIFT_EXPR:
  case LSHIFT_EXPR:
    if (TREE_CODE(type0) == INTEGER_TYPE && TREE_CODE(TREE_TYPE(type1)) == INTEGER_TYPE)
    {
  ...
  case GT_EXPR:
    ...
    if (TREE_CODE(type0) == INTEGER_TYPE && TREE_CODE(TREE_TYPE(type1)) == INTEGER_TYPE)
    {
      ...
      return ret;
             ...
             && (TREE_CODE(type0) == REAL_TYPE || TREE_CODE(type0) == INTEGER_TYPE) && SCALAR_FLOAT_TYPE_P(TREE_TYPE(type1)))
    {
      if (unsafe_conversion_p(TREE_TYPE(type1), op0,
                              NULL_TREE, false))
      {
...
bool cxx_fundamental_alignment_p(unsigned align)
{
...
bool pointer_to_zero_sized_aggr_p(tree t)
{
  if (!POINTER_TYPE_P(t))
    return false;
  t = TREE_TYPE(t);
...
bool reject_gcc_builtin(const_tree expr, location_t loc /* = UNKNOWN_LOCATION */)
{
  ...
  if (TREE_TYPE(expr) && TREE_CODE(TREE_TYPE(expr)) == FUNCTION_TYPE && TREE_CODE(expr) == FUNCTION_DECL
	 ...
	 declared built-ins with library fallbacks (such as abs).  */
      && fndecl_built_in_p(expr) && DECL_IS_BUILTIN(expr) && !c_decl_implicit(expr) && !DECL_ASSEMBLER_NAME_SET_P(expr))
  {
...
void invalid_array_size_error(location_t loc, cst_size_error error,
...
bool valid_array_size_p(location_t loc, const_tree t, tree name, bool complain)
{
  if (t == error_mark_node)
    return true;
...
#endif
  if (errno != 0 || endptr == source_date_epoch || *endptr != '\0' || epoch < 0 || epoch > MAX_SOURCE_DATE_EPOCH)
  {
...
{
  if (x == FLT_EVAL_METHOD_UNPREDICTABLE || y == FLT_EVAL_METHOD_UNPREDICTABLE)
...
{
  enum flt_eval_method implicit = targetm.c.excess_precision(EXCESS_PRECISION_TYPE_IMPLICIT);

  enum excess_precision_type flag_type = (flag_excess_precision == EXCESS_PRECISION_STANDARD
                                              ? EXCESS_PRECISION_TYPE_STANDARD
                                              : EXCESS_PRECISION_TYPE_FAST);

  enum flt_eval_method requested = targetm.c.excess_precision(flag_type);
...
int c_flt_eval_method(bool maybe_c11_only_p)
{
  if (maybe_c11_only_p && flag_permitted_flt_eval_methods == PERMITTED_FLT_EVAL_METHODS_C11)
...
void maybe_suggest_missing_token_insertion(rich_location *richloc,
                                           enum cpp_ttype token_type,
                                           location_t prev_token_loc)
{
  gcc_assert(richloc);

  enum missing_token_insertion_kind mtik = get_missing_token_insertion_kind(token_type);
...
try_to_locate_new_include_insertion_point(const char *file, location_t loc)
{
  ...
  {
    const line_map_ordinary *ord_map = LINEMAPS_ORDINARY_MAP_AT(line_table, i);
...
void maybe_add_include_fixit(rich_location *richloc, const char *header,
                             bool override_location)
{
  ...
  location_t include_insert_loc = try_to_locate_new_include_insertion_point(file, loc);
