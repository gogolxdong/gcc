...
void c_parse_init(void)
{
	...
	for (i = 0; i < num_c_common_reswords; i++)
	{
	 ...
	 and so create a canonical spelling that isn't a keyword.  */
		...
		{
			if (warn_cxx_compat && (c_common_reswords[i].disable & D_CXXWARN))
			{
...
bool c_parser_error(c_parser *parser)
{
	return parser->error;
}
...
void c_parser_set_error(c_parser *parser, bool err)
{
	parser->error = err;
}
...
c_lex_one_token(c_parser *parser, c_token *token, bool raw = false)
{
	...
	case CPP_AT_NAME:
	  ...
	  */
		case RID_CLASS:
			token->keyword = RID_AT_CLASS;
			break;
		case RID_PRIVATE:
			token->keyword = RID_AT_PRIVATE;
			break;
		case RID_PROTECTED:
			token->keyword = RID_AT_PROTECTED;
			break;
		case RID_PUBLIC:
			token->keyword = RID_AT_PUBLIC;
			break;
		case RID_THROW:
			token->keyword = RID_AT_THROW;
			break;
		case RID_TRY:
			token->keyword = RID_AT_TRY;
			break;
		case RID_CATCH:
			token->keyword = RID_AT_CATCH;
			break;
		case RID_SYNCHRONIZED:
			token->keyword = RID_AT_SYNCHRONIZED;
			break;
		default:
...
bool c_keyword_starts_typename(enum rid keyword)
{
	switch (keyword)
	{
	case RID_UNSIGNED:
	...
	case RID_ALIGNAS:
		return true;
	default:
		if (keyword >= RID_FIRST_INT_N && keyword < RID_FIRST_INT_N + NUM_INT_N_ENTS && int_n_enabled_p[keyword - RID_FIRST_INT_N])
			return true;
		return false;
	}
}
...
c_parser_next_tokens_start_typename(c_parser *parser, enum c_lookahead_kind la)
{
	...
	if (la != cla_prefer_id && token->type == CPP_NAME && token->id_kind == C_ID_ID

		/* Do not try too hard when we could have "object in array".  */
		&& !parser->objc_could_be_foreach_context

		&& (la == cla_prefer_type || c_parser_peek_2nd_token(parser)->type == CPP_NAME || c_parser_peek_2nd_token(parser)->type == CPP_MULT)
...
c_token_starts_declspecs(c_token *token)
{
	...
	case CPP_KEYWORD:
		...
		case RID_AUTO_TYPE:
			return true;
		default:
			if (token->keyword >= RID_FIRST_INT_N && token->keyword < RID_FIRST_INT_N + NUM_INT_N_ENTS && int_n_enabled_p[token->keyword - RID_FIRST_INT_N])
				return true;
			return false;
		}
	case CPP_LESS:
...
/* Return true if the next token from PARSER can start declaration
   ...
   otherwise.  */
bool c_parser_next_token_starts_declspecs(c_parser *parser)
{
	c_token *token = c_parser_peek_token(parser);
	...
	if (c_dialect_objc() && token->type == CPP_NAME && token->id_kind == C_ID_CLASSNAME && c_parser_peek_2nd_token(parser)->type == CPP_DOT)
		return false;
...
bool c_parser_next_tokens_start_declaration(c_parser *parser)
{
	...
	if (token->type == CPP_NAME && c_parser_peek_2nd_token(parser)->type == CPP_COLON)
		return false;
...
void c_parser_consume_token(c_parser *parser)
{
	gcc_assert(parser->tokens_avail >= 1);
	gcc_assert(parser->tokens[0].type != CPP_EOF);
...
bool c_parser_error(c_parser *parser, const char *gmsgid)
{
	c_token *token = c_parser_peek_token(parser);
...
bool c_parser_require(c_parser *parser,
					  enum cpp_ttype type,
					  const char *msgid,
...
void c_parser_skip_until_found(c_parser *parser,
							   enum cpp_ttype type,
							   const char *msgid,
							   location_t matching_location)
{
...
{
	int ret = (pedantic | (warn_pointer_arith << 1) | (warn_traditional << 2) | (flag_iso << 3) | (warn_long_long << 4) | (warn_cxx_compat << 5) | (warn_overlength_strings << 6)
			   /* warn_c90_c99_compat has three states: -1/0/1, so we must
		play tricks to properly restore it.  */
			   | ((warn_c90_c99_compat == 1) << 7) | ((warn_c90_c99_compat == -1) << 8)
			   /* Similarly for warn_c99_c11_compat.  */
			   | ((warn_c99_c11_compat == 1) << 9) | ((warn_c99_c11_compat == -1) << 10)
			   /* Similarly for warn_c11_c2x_compat.  */
			   | ((warn_c11_c2x_compat == 1) << 11) | ((warn_c11_c2x_compat == -1) << 12));
...
static bool c_parser_objc_diagnose_bad_element_prefix(c_parser *, struct c_declspecs *);
							  ...
							  bool *fallthru_attr_p)
{
	...
	if (c_parser_peek_token(parser)->type == CPP_NAME && c_parser_peek_token(parser)->id_kind == C_ID_ID && (c_parser_peek_2nd_token(parser)->type == CPP_NAME || c_parser_peek_2nd_token(parser)->type == CPP_MULT) && (!nested || !lookup_name(c_parser_peek_token(parser)->value)))
	{
	...
	if (empty_ok && specs->typespec_kind == ctsk_tagdef && c_parser_next_token_starts_declspecs(parser) && !c_parser_next_token_is(parser, CPP_NAME))
	{
	...
	while (true)
	{
	 ...
	 implicit int suffices).  */
		...
		if (c_parser_next_token_is(parser, CPP_EQ) || c_parser_next_token_is(parser, CPP_COMMA) || c_parser_next_token_is(parser, CPP_SEMICOLON) || c_parser_next_token_is_keyword(parser, RID_ASM) || c_parser_next_token_is_keyword(parser, RID_ATTRIBUTE) || c_parser_next_token_is_keyword(parser, RID_IN))
		{
						...
						enum c_lookahead_kind la)
{
	...
	while (c_parser_next_token_is(parser, CPP_NAME) || c_parser_next_token_is(parser, CPP_KEYWORD) || (c_dialect_objc() && c_parser_next_token_is(parser, CPP_LESS)))
	{
		struct c_typespec t;
		tree attrs;
		tree align;
	 ...
	 has simply forgotten a semicolon, so we exit.  */
		if ((!typespec_ok || specs->typespec_kind == ctsk_tagdef) && c_parser_next_tokens_start_typename(parser, la) && !c_parser_next_token_is_qualifier(parser) && !c_parser_next_token_is_keyword(parser, RID_ALIGNAS))
			break;
...
c_parser_struct_declaration(c_parser *parser)
{
	...
	if (specs->typespec_kind == ctsk_tagdef && TREE_CODE(specs->type) != ENUMERAL_TYPE && c_parser_next_token_starts_declspecs(parser) && !c_parser_next_token_is(parser, CPP_NAME))
	{
		c_parser_error(parser, "expected %<;%>, identifier or %<(%>");
		parser->error = false;
		return NULL_TREE;
	...
	while (true)
	{
		...
		if (c_parser_next_token_is(parser, CPP_COLON) || c_parser_next_token_is(parser, CPP_COMMA) || c_parser_next_token_is(parser, CPP_SEMICOLON) || c_parser_next_token_is(parser, CPP_CLOSE_BRACE) || c_parser_next_token_is_keyword(parser, RID_ATTRIBUTE))
		{
			tree postfix_attrs = NULL_TREE;
						   ...
						   bool *seen_id)
{
	...
	if (kind != C_DTR_ABSTRACT && c_parser_next_token_is(parser, CPP_NAME) && ((type_seen_p && (c_parser_peek_token(parser)->id_kind == C_ID_TYPENAME || c_parser_peek_token(parser)->id_kind == C_ID_CLASSNAME)) || c_parser_peek_token(parser)->id_kind == C_ID_ID))
	{
	...
	if (kind != C_DTR_NORMAL && c_parser_next_token_is(parser, CPP_OPEN_SQUARE) && !c_parser_nth_token_starts_std_attributes(parser, 1))
	{
	...
	if (c_parser_next_token_is(parser, CPP_OPEN_PAREN))
	{
		...
		bool have_gnu_attrs = c_parser_next_token_is_keyword(parser,
															 RID_ATTRIBUTE);
		attrs = c_parser_gnu_attributes(parser);
		if (kind != C_DTR_NORMAL && (c_parser_next_token_starts_declspecs(parser) || (!have_gnu_attrs && c_parser_nth_token_starts_std_attributes(parser, 1)) || c_parser_next_token_is(parser, CPP_CLOSE_PAREN)))
		{
			struct c_arg_info *args = c_parser_parms_declarator(parser, kind == C_DTR_NORMAL,
			...
			else
			{
				...
				if (!(args->types && args->types != error_mark_node && TREE_CODE(TREE_VALUE(args->types)) == IDENTIFIER_NODE) && c_parser_nth_token_starts_std_attributes(parser, 1))
				{
						  ...
						  bool have_gnu_attrs)
{
	...
	if (id_list_ok && !attrs && c_parser_next_token_is(parser, CPP_NAME) && c_parser_peek_token(parser)->id_kind == C_ID_ID

		/* Look ahead to detect typos in type names.  */
		&& c_parser_peek_2nd_token(parser)->type != CPP_NAME && c_parser_peek_2nd_token(parser)->type != CPP_MULT && c_parser_peek_2nd_token(parser)->type != CPP_OPEN_PAREN && c_parser_peek_2nd_token(parser)->type != CPP_OPEN_SQUARE && c_parser_peek_2nd_token(parser)->type != CPP_KEYWORD)
	{
					   ...
					   bool expect_comma = false, bool empty_ok = true)
{
	...
	if (!comma_first && !c_parser_next_token_is(parser, CPP_NAME) && !c_parser_next_token_is(parser, CPP_KEYWORD))
		return NULL_TREE;
...
c_parser_std_attribute(c_parser *parser, bool for_tm)
{
		...
		goto out;
	{
	   ...
	   sequences as arguments.  */
		...
		if (as)
		{
			bool takes_identifier = (ns != NULL_TREE && strcmp(IDENTIFIER_POINTER(ns), "gnu") == 0 && attribute_takes_identifier_p(name));
			bool require_string = (ns == NULL_TREE && strcmp(IDENTIFIER_POINTER(name), "deprecated") == 0);
			TREE_VALUE(attribute) = c_parser_attribute_arguments(parser, takes_identifier,
...
{
	if (!(c_parser_peek_nth_token(parser, n)->type == CPP_OPEN_SQUARE && c_parser_peek_nth_token(parser, n + 1)->type == CPP_OPEN_SQUARE))
		return false;
...
c_parser_initelt(c_parser *parser, struct obstack *braced_init_obstack)
{
	...
	if (c_parser_next_token_is(parser, CPP_NAME) && c_parser_peek_2nd_token(parser)->type == CPP_COLON)
	{
	...
	else
	{
	 ...
	 has been a single array designator and 2 otherwise.  */
		...
		while (c_parser_next_token_is(parser, CPP_OPEN_SQUARE) || c_parser_next_token_is(parser, CPP_DOT))
		{
		 ...
		 designator to designator-list.  */
		     ...
		     previous comment.  */
					...
					struct c_expr mexpr;
					c_parser_consume_token(parser);
					if (c_parser_peek_token(parser)->type == CPP_NAME && ((c_parser_peek_token(parser)->id_kind == C_ID_TYPENAME) || (c_parser_peek_token(parser)->id_kind == C_ID_CLASSNAME)))
					{
						/* Type name receiver.  */
					...
					mark_exp_read(first);
					if (c_parser_next_token_is(parser, CPP_ELLIPSIS) || c_parser_next_token_is(parser, CPP_CLOSE_SQUARE))
		     ...
		     more of the expression.  */
				...
				parse_message_args:
					...
					c_parser_skip_until_found(parser, CPP_CLOSE_SQUARE,
											  "expected %<]%>");
					mexpr.value = objc_build_message_expr(rec, args);
				 ...
				 struct obstack *braced_init_obstack)
{
	...
	else
	{
		...
		if (init.value != NULL_TREE && TREE_CODE(init.value) != STRING_CST && TREE_CODE(init.value) != COMPOUND_LITERAL_EXPR)
...
c_parser_compound_statement_nostart(c_parser *parser)
{
	...
	while (c_parser_next_token_is_not(parser, CPP_CLOSE_BRACE))
	{
		...
		if (have_std_attrs)
			std_attrs = c_parser_std_attribute_specifier_sequence(parser);
		if (c_parser_next_token_is_keyword(parser, RID_CASE) || c_parser_next_token_is_keyword(parser, RID_DEFAULT) || (c_parser_next_token_is(parser, CPP_NAME) && c_parser_peek_2nd_token(parser)->type == CPP_COLON))
		{
		...
		else if (!last_label && (c_parser_next_tokens_start_declaration(parser) || (have_std_attrs && c_parser_next_token_is(parser, CPP_SEMICOLON))))
		{
			last_label = false;
			mark_valid_location_for_stdc_pragma(false);
		...
		else if (!last_label && c_parser_next_token_is_keyword(parser, RID_EXTENSION))
		{
	     ...
	     after __extension__.  */
			while (c_parser_peek_2nd_token(parser)->type == CPP_KEYWORD && (c_parser_peek_2nd_token(parser)->keyword == RID_EXTENSION))
				c_parser_consume_token(parser);
			if (!have_std_attrs && (c_token_starts_declaration(c_parser_peek_2nd_token(parser)) || c_parser_nth_token_starts_std_attributes(parser, 2)))
			{
				int ext;
				ext = disable_extension_diagnostics();
				c_parser_consume_token(parser);
...
c_parser_all_labels(c_parser *parser)
{
	...
	while (c_parser_next_token_is_keyword(parser, RID_CASE) || c_parser_next_token_is_keyword(parser, RID_DEFAULT) || (c_parser_next_token_is(parser, CPP_NAME) && c_parser_peek_2nd_token(parser)->type == CPP_COLON))
		c_parser_label(parser);
}
...
c_parser_label(c_parser *parser)
{
	...
	if (label)
	{
	 ...
	 attribute-declaration with only attributes).  */
		...
		if (!have_std_attrs && c_parser_next_token_is_keyword(parser, RID_ATTRIBUTE))
		{
			location_t loc = c_parser_peek_token(parser)->location;
		...
		if (c_parser_next_tokens_start_declaration(parser) || (have_std_attrs && c_parser_next_token_is(parser, CPP_SEMICOLON)))
		{
			error_at(c_parser_peek_token(parser)->location,
				 ...
				 const token_indent_info &if_tinfo)
{
	...
	if (body_loc_after_labels != UNKNOWN_LOCATION && next_tinfo.type != CPP_SEMICOLON)
...
c_parser_if_statement(c_parser *parser, bool *if_p, vec<tree> *chain)
{
	...
	token_indent_info if_tinfo = get_token_indent_info(c_parser_peek_token(parser));
	c_parser_consume_token(parser);
	...
	if (c_parser_next_token_is_keyword(parser, RID_ELSE))
	{
		token_indent_info else_tinfo = get_token_indent_info(c_parser_peek_token(parser));
		c_parser_consume_token(parser);
		if (warn_duplicated_cond)
		{
			if (c_parser_next_token_is_keyword(parser, RID_IF) && chain == NULL)
			{
						 ...
						 bool *if_p)
{
	...
	token_indent_info while_tinfo = get_token_indent_info(c_parser_peek_token(parser));
	c_parser_consume_token(parser);
	block = c_begin_compound_stmt(flag_isoc99);
	loc = c_parser_peek_token(parser)->location;
	cond = c_parser_paren_condition(parser);
					   ...
					   bool *if_p)
{
	...
	token_indent_info for_tinfo = get_token_indent_info(c_parser_peek_token(parser));
	c_parser_consume_token(parser);
	...
	if (parens.require_open(parser))
	{
		...
		else if (c_parser_next_tokens_start_declaration(parser) || c_parser_nth_token_starts_std_attributes(parser, 1))
		{
		...
		else if (c_parser_next_token_is_keyword(parser, RID_EXTENSION))
		{
	     ...
	     determine which.  */
			while (c_parser_peek_2nd_token(parser)->type == CPP_KEYWORD && (c_parser_peek_2nd_token(parser)->keyword == RID_EXTENSION))
				c_parser_consume_token(parser);
			if (c_token_starts_declaration(c_parser_peek_2nd_token(parser)) || c_parser_nth_token_starts_std_attributes(parser, 2))
			{
				int ext;
				ext = disable_extension_diagnostics();
				c_parser_consume_token(parser);
...
c_parser_asm_statement(c_parser *parser)
{
	...
	for (section = 0; section < nsections; ++section)
	{
		...
		if ((!c_parser_next_token_is(parser, CPP_COLON) && !c_parser_next_token_is(parser, CPP_SCOPE) && !c_parser_next_token_is(parser, CPP_CLOSE_PAREN)) || section == 3)
			switch (section)
			{
			case 0:
...
c_parser_string_literal(c_parser *parser, bool translate, bool wide_ok)
{
	...
	case CPP_UTF8STRING:
		...
		do
		{
			...
			tok = c_parser_peek_token(parser);
			string_tree = tok->value;
			last_tok_loc = linemap_resolve_location(line_table, tok->location,
													LRK_MACRO_DEFINITION_LOCATION, NULL);
		} while (tok->type == CPP_STRING || tok->type == CPP_WSTRING || tok->type == CPP_STRING16 || tok->type == CPP_STRING32 || tok->type == CPP_UTF8STRING);
	...
	if ((type == CPP_STRING || wide_ok) && ((translate
												 ? cpp_interpret_string
												 : cpp_interpret_string_notranslate)(parse_in, strs, count, &istr, type)))
	{
	...
	else
	{
	 ...
	 been issued, either above or from cpp_interpret_string.  */
		...
		case CPP_UTF8STRING:
			...
			break;
		case CPP_STRING16:
			value = build_string(TYPE_PRECISION(char16_type_node) / TYPE_PRECISION(char_type_node),
								 "\0"); /* char16_t is 16 bits */
			break;
		case CPP_STRING32:
			value = build_string(TYPE_PRECISION(char32_type_node) / TYPE_PRECISION(char_type_node),
								 "\0\0\0"); /* char32_t is 32 bits */
			break;
		case CPP_WSTRING:
			value = build_string(TYPE_PRECISION(wchar_type_node) / TYPE_PRECISION(char_type_node),
								...
								tree omp_atomic_lhs)
{
	...
	else
	{
		...
		c_inhibit_evaluation_warnings +=
			((cond.value == truthvalue_true_node) - (cond.value == truthvalue_false_node));
	}
	...
	else
	{
	 ...
	 remember the enum types we started with.  */
		...
		ret.original_type = ((t1 != error_mark_node && t2 != error_mark_node && (TYPE_MAIN_VARIANT(t1) == TYPE_MAIN_VARIANT(t2)))
								 ? t1
								 : NULL);
	}
...
#define POP                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          \
	...
	{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
		...
		case TRUTH_ANDIF_EXPR:                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       \
			c_inhibit_evaluation_warnings -= (stack[sp - 1].expr.value == truthvalue_false_node);                                                                                                                                                                                                                                                                                                                                                                                                                    \
			break;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
		case TRUTH_ORIF_EXPR:                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
			c_inhibit_evaluation_warnings -= (stack[sp - 1].expr.value == truthvalue_true_node);                                                                                                                                                                                                                                                                                                                                                                                                                     \
			break;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
		case TRUNC_DIV_EXPR:                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
			if (stack[sp - 1].expr.original_code == SIZEOF_EXPR && stack[sp].expr.original_code == SIZEOF_EXPR)                                                                                                                                                                                                                                                                                                                                                                                                      \
			{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        \
				...
				if (POINTER_TYPE_P(type0) && comptypes(TREE_TYPE(type0), type1) && !(TREE_CODE(first_arg) == PARM_DECL && C_ARRAY_PARAMETER(first_arg) && warn_sizeof_array_argument))                                                                                                                                                                                                                                                                                                                               \
				{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
					auto_diagnostic_group d;                                                                                                                                                                                                                                                                                                                                                                                                                                                                         \
		...
		stack[sp - 1].expr = convert_lvalue_to_rvalue(stack[sp - 1].loc,                                                                                                                                                                                                                                                                                                                                                                                                                                             \
													  stack[sp - 1].expr, true, true);                                                                                                                                                                                                                                                                                                                                                                                                                               \
		stack[sp].expr = convert_lvalue_to_rvalue(stack[sp].loc,                                                                                                                                                                                                                                                                                                                                                                                                                                                     \
												  stack[sp].expr, true, true);                                                                                                                                                                                                                                                                                                                                                                                                                                       \
		if (__builtin_expect(omp_atomic_lhs != NULL_TREE, 0) && sp == 1 && c_parser_peek_token(parser)->type == CPP_SEMICOLON && ((1 << stack[sp].prec) & ((1 << PREC_BITOR) | (1 << PREC_BITXOR) | (1 << PREC_BITAND) | (1 << PREC_SHIFT) | (1 << PREC_ADD) | (1 << PREC_MULT))) && stack[sp].op != TRUNC_MOD_EXPR && stack[0].expr.value != error_mark_node && stack[1].expr.value != error_mark_node && (c_tree_equal(stack[0].expr.value, omp_atomic_lhs) || c_tree_equal(stack[1].expr.value, omp_atomic_lhs))) \
			stack[0].expr.value = build2(stack[1].op, TREE_TYPE(stack[0].expr.value),                                                                                                                                                                                                                                                                                                                                                                                                                                \
...
c_parser_alignof_expression(c_parser *parser)
{
	...
	if (c_parser_next_token_is(parser, CPP_OPEN_PAREN) && c_token_starts_typename(c_parser_peek_2nd_token(parser)))
	{
	 ...
	 unary-expression starting with a compound literal.  */
		...
		c_inhibit_evaluation_warnings--;
		in_alignof--;
		ret.value = c_sizeof_or_alignof_type(loc, groktypename(type_name, NULL, NULL),
						  ...
						  location_t *out_close_paren_loc)
{
	...
	*ret_cexpr_list = NULL;
	...
	saved_force_folding_builtin_constant_p = force_folding_builtin_constant_p;
	force_folding_builtin_constant_p |= choose_expr_p;
	expr = c_parser_expr_no_commas(parser, NULL);
	force_folding_builtin_constant_p = saved_force_folding_builtin_constant_p;
...
c_parser_generic_selection(c_parser *parser)
{
	...
	/* In ISO C terms, _Noreturn is not part of the type of expressions
     ...
     qualifier.  */
	if (FUNCTION_POINTER_TYPE_P(selector_type) && TYPE_QUALS(TREE_TYPE(selector_type)) != TYPE_UNQUALIFIED)
		selector_type = build_pointer_type(TYPE_MAIN_VARIANT(TREE_TYPE(selector_type)));
	...
	case CPP_NAME:
		...
		case C_ID_ID:
		{
			...
			expr.value = build_external_ref(loc, id,
											(c_parser_peek_token(parser)->type == CPP_OPEN_PAREN),
											&expr.original_type);
			set_c_expr_source_range(&expr, tok_range);
			break;
		}
	...
	case CPP_KEYWORD:
		...
		case RID_OFFSETOF:
		{
	       ...
	       accept sub structure and sub array references.  */
			if (c_parser_next_token_is(parser, CPP_NAME))
			{
				...
				c_parser_consume_token(parser);
				while (c_parser_next_token_is(parser, CPP_DOT) || c_parser_next_token_is(parser, CPP_OPEN_SQUARE) || c_parser_next_token_is(parser, CPP_DEREF))
				{
					...
					do_dot:
						...
						c_token *comp_tok = c_parser_peek_token(parser);
						offsetof_ref = build_component_ref(loc, offsetof_ref, comp_tok->value,
														   comp_tok->location);
						c_parser_consume_token(parser);
					}
					else
		...
		case RID_BUILTIN_TGMATH:
		{
				...
				if (DECIMAL_FLOAT_TYPE_P(type))
				{
					...
					if (all_complex)
					{
						error_at(ce->get_location(),
								 ...
								 "complex-only type-generic function",
								 j);
						expr.set_error();
						goto out;
					}
					else if (all_binary)
					{
						error_at(ce->get_location(),
								 "decimal floating-point argument %u to "
								 "binary-only type-generic function",
								 j);
						expr.set_error();
						goto out;
					}
				...
				else if (TREE_CODE(type) == COMPLEX_TYPE)
				{
					...
					if (all_decimal)
					{
						error_at(ce->get_location(),
								 ...
								 "decimal-only type-generic function",
								 j);
						expr.set_error();
						goto out;
					}
	       ...
	       is a _FloatN or _FloatNx type.  */
			...
			if (parm_kind[0] == tgmath_fixed && SCALAR_FLOAT_TYPE_P(parm_first[0]) && float64_type_node != NULL_TREE)
		   ...
		   type.  */
				for (unsigned int j = 0; j < num_functions; j++)
				{
					...
					if (arg_fmt->b == tg_fmt->b && arg_fmt->p <= tg_fmt->p && arg_fmt->emax <= tg_fmt->emax && (arg_fmt->emin - arg_fmt->p >= tg_fmt->emin - tg_fmt->p))
					{
						fn = &(*cexpr_list)[j];
						break;
					}
		...
		case RID_BUILTIN_COMPLEX:
		{
			...
			*e2_p = convert_lvalue_to_rvalue(loc, *e2_p, true, true);
			...
			if (!SCALAR_FLOAT_TYPE_P(TREE_TYPE(e1_p->value)) || DECIMAL_FLOAT_TYPE_P(TREE_TYPE(e1_p->value)) || !SCALAR_FLOAT_TYPE_P(TREE_TYPE(e2_p->value)) || DECIMAL_FLOAT_TYPE_P(TREE_TYPE(e2_p->value)))
			{
			...
			expr.value = build2_loc(loc, COMPLEX_EXPR,
									build_complex_type(TYPE_MAIN_VARIANT(TREE_TYPE(e1_p->value))),
											 ...
											 location_t type_loc)
{
	...
	if (type != error_mark_node && !ADDR_SPACE_GENERIC_P(TYPE_ADDR_SPACE(type)) && current_function_decl)
	{
	...
	if (type != error_mark_node && type_name->specs->align_log != -1)
	{
	...
	expr.original_code = ERROR_MARK;
	expr.original_type = NULL;
	if (type != error_mark_node && expr.value != error_mark_node && type_expr)
	{
	...
	case BUILT_IN_IMAXABS:
		if (!INTEGRAL_TYPE_P(atype))
		{
			if (SCALAR_FLOAT_TYPE_P(atype))
				...
				warning_at(loc, OPT_Wabsolute_value,
						   "using integer absolute value function %qD when "
						   "argument is of complex type %qT",
						   fndecl, atype);
			else
				gcc_unreachable();
			return;
		...
		CASE_FLT_FN(BUILT_IN_FABS) : CASE_FLT_FN_FLOATN_NX(BUILT_IN_FABS) : if (!SCALAR_FLOAT_TYPE_P(atype) || DECIMAL_FLOAT_MODE_P(TYPE_MODE(atype)))
		{
			if (INTEGRAL_TYPE_P(atype))
				warning_at(loc, OPT_Wabsolute_value,
						   "using floating-point absolute value function %qD "
						   "when argument is of integer type %qT",
						   fndecl, atype);
		...
		CASE_FLT_FN(BUILT_IN_CABS) : if (TREE_CODE(atype) != COMPLEX_TYPE)
		{
			if (INTEGRAL_TYPE_P(atype))
				warning_at(loc, OPT_Wabsolute_value,
						   "using complex absolute value function %qD when "
						   "argument is of integer type %qT",
						   fndecl, atype);
										  ...
										  struct c_expr expr)
{
	...
	while (true)
	{
		...
		case CPP_OPEN_PAREN:
			...
			expr.original_code = ERROR_MARK;
			if (TREE_CODE(expr.value) == INTEGER_CST && TREE_CODE(orig_expr.value) == FUNCTION_DECL && fndecl_built_in_p(orig_expr.value, BUILT_IN_CONSTANT_P))
							...
							unsigned int idx)
{
	...
	case CPP_UTF8CHAR:
	 ...
	 for -Wmemset-transposed-args warning.  */
		if (integer_zerop(tok->value) && !TREE_OVERFLOW(tok->value) && (c_parser_peek_2nd_token(parser)->type == CPP_COMMA || c_parser_peek_2nd_token(parser)->type == CPP_CLOSE_PAREN))
...
c_parser_objc_protocol_definition(c_parser *parser, tree attributes)
{
	...
	if (c_parser_peek_2nd_token(parser)->type == CPP_COMMA || c_parser_peek_2nd_token(parser)->type == CPP_SEMICOLON)
	{
...
c_parser_objc_maybe_method_attributes(c_parser *parser, tree *attributes)
{
	...
	/* If the attributes are followed by a ; or {, then just report any earlier
     error.  */
	if (c_parser_next_token_is(parser, CPP_SEMICOLON) || c_parser_next_token_is(parser, CPP_OPEN_BRACE))
		return bad;
...
c_parser_objc_type_name(c_parser *parser)
{
	...
	{
		c_token *token = c_parser_peek_token(parser);
		if (token->type == CPP_KEYWORD && (token->keyword == RID_IN || token->keyword == RID_OUT || token->keyword == RID_INOUT || token->keyword == RID_BYCOPY || token->keyword == RID_BYREF || token->keyword == RID_ONEWAY))
		{
...
c_parser_objc_selector_arg(c_parser *parser)
{
	...
	tree list = NULL_TREE;
	if (sel && c_parser_next_token_is_not(parser, CPP_COLON) && c_parser_next_token_is_not(parser, CPP_SCOPE))
		return sel;
	while (true)
	{
		if (c_parser_next_token_is(parser, CPP_SCOPE))
...
c_parser_objc_diagnose_bad_element_prefix(c_parser *parser,
										  struct c_declspecs *specs)
{
	if (!specs->declspecs_seen_p || specs->non_sc_seen_p || specs->typespec_kind != ctsk_none)
	{
		c_parser_error(parser,
...
c_parser_pragma(c_parser *parser, enum pragma_context context, bool *if_p)
{
	...
	case PRAGMA_IVDEP:
	{
		...
		if (c_parser_peek_token(parser)->pragma_kind == PRAGMA_UNROLL)
			...
			unroll = 0;
		if (!c_parser_next_token_is_keyword(parser, RID_FOR) && !c_parser_next_token_is_keyword(parser, RID_WHILE) && !c_parser_next_token_is_keyword(parser, RID_DO))
		{
						   ...
						   bool allow_deref = false)
{
	...
	while (1)
	{
		...
		{
			if (c_parser_next_token_is_not(parser, CPP_NAME) || c_parser_peek_token(parser)->id_kind != C_ID_ID)
			{
		...
		else if (c_parser_next_token_is(parser, CPP_KEYWORD) && (c_parser_peek_token(parser)->keyword == RID_FUNCTION_NAME || (c_parser_peek_token(parser)->keyword == RID_PRETTY_FUNCTION_NAME) || (c_parser_peek_token(parser)->keyword == RID_C99_FUNCTION_NAME)))
		...
		else if (kind != 0)
		{
		 ...
		 elements or subarrays".  */
			...
			case OMP_CLAUSE_TO:
				while (c_parser_next_token_is(parser, CPP_DOT) || (allow_deref && c_parser_next_token_is(parser, CPP_DEREF)))
				{
					location_t op_loc = c_parser_peek_token(parser)->location;
					if (c_parser_next_token_is(parser, CPP_DEREF))
			...
			case OMP_CLAUSE_TASK_REDUCTION:
				...
				if (kind == OMP_CLAUSE_DEPEND && t != error_mark_node && parser->tokens_avail != 2)
				{
					if (array_section_p)
					{
						error_at(c_parser_peek_token(parser)->location,
...
c_parser_omp_clause_collapse(c_parser *parser, tree list)
{
	...
	if (!INTEGRAL_TYPE_P(TREE_TYPE(num)) || !tree_fits_shwi_p(num) || (n = tree_to_shwi(num)) <= 0 || (int)n != n)
	{
		error_at(loc,
...
c_parser_omp_clause_if(c_parser *parser, tree list, bool is_omp)
{
	...
	if (is_omp && c_parser_next_token_is(parser, CPP_NAME))
	{
		...
		else if (strcmp(p, "target") == 0)
		{
			...
			if (c_parser_peek_2nd_token(parser)->type == CPP_NAME)
			{
				...
				if (if_modifier == OMP_TARGET_ENTER_DATA || if_modifier == OMP_TARGET_EXIT_DATA)
				{
					if (c_parser_peek_2nd_token(parser)->type == CPP_NAME)
					{
						p = IDENTIFIER_POINTER(c_parser_peek_2nd_token(parser)->value);
	...
	for (c = list; c; c = OMP_CLAUSE_CHAIN(c))
		...
		{
			if (if_modifier != ERROR_MARK && OMP_CLAUSE_IF_MODIFIER(c) == if_modifier)
			{
				const char *p = NULL;
				switch (if_modifier)
				{
			...
			else if (if_modifier == ERROR_MARK || OMP_CLAUSE_IF_MODIFIER(c) == ERROR_MARK)
			{
...
c_parser_omp_clause_hint(c_parser *parser, tree list)
{
	...
	if (parens.require_open(parser))
	{
		...
		if (!INTEGRAL_TYPE_P(TREE_TYPE(t)) || TREE_CODE(t) != INTEGER_CST)
		{
...
c_parser_omp_clause_defaultmap(c_parser *parser, tree list)
{
	...
	enum omp_clause_defaultmap_kind category = OMP_CLAUSE_DEFAULTMAP_CATEGORY_UNSPECIFIED;
	...
	for (c = list; c; c = OMP_CLAUSE_CHAIN(c))
		if (OMP_CLAUSE_CODE(c) == OMP_CLAUSE_DEFAULTMAP && (category == OMP_CLAUSE_DEFAULTMAP_CATEGORY_UNSPECIFIED || OMP_CLAUSE_DEFAULTMAP_CATEGORY(c) == category || (OMP_CLAUSE_DEFAULTMAP_CATEGORY(c) == OMP_CLAUSE_DEFAULTMAP_CATEGORY_UNSPECIFIED)))
		{
						   ...
						   const char *str, tree list)
{
	...
	if (c_parser_next_token_is(parser, CPP_OPEN_PAREN))
	{
		...
		do
		{
			...
			if (kind == OMP_CLAUSE_GANG && c_parser_next_token_is_keyword(parser, RID_STATIC))
			{
				c_parser_consume_token(parser);
				...
				if (c_parser_next_token_is(parser, CPP_MULT) && (c_parser_peek_2nd_token(parser)->type == CPP_COMMA || c_parser_peek_2nd_token(parser)->type == CPP_CLOSE_PAREN))
				{
					c_parser_consume_token(parser);
			...
			else if (c_parser_next_token_is(parser, CPP_NAME) && strcmp(id, IDENTIFIER_POINTER(next->value)) == 0 && c_parser_peek_2nd_token(parser)->type == CPP_COLON)
			{
	     ...
	     positive integer.  */
			...
			if (kind == OMP_CLAUSE_GANG && c_parser_next_token_is(parser, CPP_COMMA))
			{
				c_parser_consume_token(parser);
				continue;
			}
...
c_parser_oacc_clause_async(c_parser *parser, tree list)
{
	...
	if (c_parser_peek_token(parser)->type == CPP_OPEN_PAREN)
	{
		...
		if (!INTEGRAL_TYPE_P(TREE_TYPE(t)))
			c_parser_error(parser, "expected integer expression");
		else if (t == error_mark_node || !c_parser_require(parser, CPP_CLOSE_PAREN, "expected %<)%>"))
			return list;
	}
	else
		t = c_fully_fold(t, false, NULL);
...
c_parser_oacc_clause_tile(c_parser *parser, tree list)
{
	...
	do
	{
		...
		else
		{
			...
			if (!INTEGRAL_TYPE_P(TREE_TYPE(expr)) || !tree_fits_shwi_p(expr) || tree_to_shwi(expr) <= 0)
			{
							  ...
							  bool is_omp, tree list)
{
	...
	if (parens.require_open(parser))
	{
		...
		{
			if (c_parser_next_token_is_keyword(parser, RID_DEFAULT) && c_parser_peek_2nd_token(parser)->type == CPP_COMMA)
			{
				c_parser_consume_token(parser);
				c_parser_consume_token(parser);
			}
			else if (c_parser_next_token_is(parser, CPP_NAME) && c_parser_peek_2nd_token(parser)->type == CPP_COMMA)
			{
				const char *p = IDENTIFIER_POINTER(c_parser_peek_token(parser)->value);
				if (strcmp(p, "task") == 0)
					task = true;
		...
		if (c_parser_require(parser, CPP_COLON, "expected %<:%>"))
		{
			...
			for (c = nl; c != list; c = OMP_CLAUSE_CHAIN(c))
			{
				...
				if (code == ERROR_MARK || !(INTEGRAL_TYPE_P(type) || TREE_CODE(type) == REAL_TYPE || TREE_CODE(type) == COMPLEX_TYPE))
					OMP_CLAUSE_REDUCTION_PLACEHOLDER(c) = c_omp_reduction_lookup(reduc_id,
...
c_parser_omp_clause_schedule(c_parser *parser, tree list)
{
	...
	while (c_parser_next_token_is(parser, CPP_NAME))
	{
		...
		if (strcmp("simd", p) == 0)
			...
			break;
		c_parser_consume_token(parser);
		if (nmodifiers++ == 0 && c_parser_next_token_is(parser, CPP_COMMA))
			c_parser_consume_token(parser);
		else
		{
	...
	if ((modifiers & (OMP_CLAUSE_SCHEDULE_MONOTONIC | OMP_CLAUSE_SCHEDULE_NONMONOTONIC)) == (OMP_CLAUSE_SCHEDULE_MONOTONIC | OMP_CLAUSE_SCHEDULE_NONMONOTONIC))
	{
	...
	OMP_CLAUSE_SCHEDULE_KIND(c) = (enum omp_clause_schedule_kind)(OMP_CLAUSE_SCHEDULE_KIND(c) | modifiers);
...
c_parser_omp_clause_aligned(c_parser *parser, tree list)
{
	...
	if (c_parser_next_token_is(parser, CPP_COLON))
	{
		...
		if (TREE_CODE(alignment) != INTEGER_CST || !INTEGRAL_TYPE_P(TREE_TYPE(alignment)) || tree_int_cst_sgn(alignment) != 1)
		{
...
c_parser_omp_clause_depend(c_parser *parser, tree list)
{
	...
	if (iterators && (kind == OMP_CLAUSE_DEPEND_SOURCE || kind == OMP_CLAUSE_DEPEND_SINK))
	{
		pop_scope();
	...
	if (c_parser_next_token_is(parser, CPP_NAME))
	{
		...
		if (strcmp("always", p) == 0)
		{
			...
			else if (sectok->type == CPP_NAME)
			{
				...
				if (strcmp("alloc", p) == 0 || strcmp("to", p) == 0 || strcmp("from", p) == 0 || strcmp("tofrom", p) == 0 || strcmp("release", p) == 0 || strcmp("delete", p) == 0)
				{
					c_parser_consume_token(parser);
					always = 1;
				}
						  ...
						  const char *where, bool finish_p = true)
{
	...
	while (c_parser_next_token_is_not(parser, CPP_PRAGMA_EOL))
	{
		...
		case PRAGMA_OACC_CLAUSE_REDUCTION:
			clauses = c_parser_omp_clause_reduction(parser, OMP_CLAUSE_REDUCTION,
													false, clauses);
			c_name = "reduction";
			break;
						 ...
						 int nested = 0)
{
	...
	while (c_parser_next_token_is_not(parser, CPP_PRAGMA_EOL))
	{
		...
		case PRAGMA_OMP_CLAUSE_IN_REDUCTION:
			clauses = c_parser_omp_clause_reduction(parser, OMP_CLAUSE_IN_REDUCTION,
													true, clauses);
			c_name = "in_reduction";
			break;
		...
		case PRAGMA_OMP_CLAUSE_TASK_REDUCTION:
			clauses = c_parser_omp_clause_reduction(parser, OMP_CLAUSE_TASK_REDUCTION,
													true, clauses);
		...
		case PRAGMA_OMP_CLAUSE_PARALLEL:
			clauses = c_parser_omp_clause_cancelkind(parser, OMP_CLAUSE_PARALLEL,
													 clauses);
			c_name = "parallel";
			if (!first)
			{
		...
		case PRAGMA_OMP_CLAUSE_FOR:
			clauses = c_parser_omp_clause_cancelkind(parser, OMP_CLAUSE_FOR,
													 clauses);
			c_name = "for";
			if (!first)
		...
		case PRAGMA_OMP_CLAUSE_SECTIONS:
			clauses = c_parser_omp_clause_cancelkind(parser, OMP_CLAUSE_SECTIONS,
													 clauses);
			c_name = "sections";
			if (!first)
				goto clause_not_first;
		...
		case PRAGMA_OMP_CLAUSE_TASKGROUP:
			clauses = c_parser_omp_clause_cancelkind(parser, OMP_CLAUSE_TASKGROUP,
													 clauses);
			c_name = "taskgroup";
			if (!first)
				goto clause_not_first;
		...
		case PRAGMA_OMP_CLAUSE_LINK:
			clauses = c_parser_omp_var_list_parens(parser, OMP_CLAUSE_LINK, clauses);
			c_name = "link";
			break;
		...
		case PRAGMA_OMP_CLAUSE_THREADS:
			clauses = c_parser_omp_clause_orderedkind(parser, OMP_CLAUSE_THREADS,
													  clauses);
			c_name = "threads";
			break;
		case PRAGMA_OMP_CLAUSE_SIMD:
			clauses = c_parser_omp_clause_orderedkind(parser, OMP_CLAUSE_SIMD,
													  clauses);
			c_name = "simd";
			break;
		default:
...
c_parser_oacc_declare(c_parser *parser)
{
	...
	for (t = clauses; t; t = OMP_CLAUSE_CHAIN(t))
	{
		...
		case GOMP_MAP_LINK:
			if (!global_bindings_p() && (TREE_STATIC(decl) || !DECL_EXTERNAL(decl)))
			{
				error_at(loc,
		...
		if (lookup_attribute("omp declare target", DECL_ATTRIBUTES(decl)) || lookup_attribute("omp declare target link",
...
#define OACC_ENTER_DATA_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_IF) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_ASYNC) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_ATTACH) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_COPYIN) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_CREATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_WAIT))

#define OACC_EXIT_DATA_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_IF) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_ASYNC) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_COPYOUT) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_DELETE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_DETACH) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_FINALIZE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_WAIT))
...
#define OACC_HOST_DATA_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_USE_DEVICE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_IF) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_IF_PRESENT))
...
#define OACC_ROUTINE_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_GANG) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_WORKER) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_VECTOR) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_SEQ))
...
c_parser_oacc_routine(c_parser *parser, enum pragma_context context)
{
	...
	if (c_parser_next_token_is(parser, CPP_OPEN_PAREN))
	{
		...
		if (name_token->type == CPP_NAME && (name_token->id_kind == C_ID_ID || name_token->id_kind == C_ID_TYPENAME))
		{
		...
		if (!decl || !c_parser_require(parser, CPP_CLOSE_PAREN, "expected %<)%>"))
		{
			c_parser_skip_to_pragma_eol(parser, false);
			return;
		}

		data.clauses = c_parser_oacc_all_clauses(parser, OACC_ROUTINE_CLAUSE_MASK,
												 "#pragma acc routine");
	...
	else /* No optional '( name )'.  */
	{
	 ...
	 definition", and the static assertion counts an one.  */
		if (c_parser_next_token_is(parser, CPP_PRAGMA) || c_parser_next_token_is_keyword(parser, RID_STATIC_ASSERT))
		{
			error_at(data.loc,
		...
		if (c_parser_next_token_is(parser, CPP_KEYWORD) && c_parser_peek_token(parser)->keyword == RID_EXTENSION)
		{
			int ext = disable_extension_diagnostics();
			do
				c_parser_consume_token(parser);
			while (c_parser_next_token_is(parser, CPP_KEYWORD) && c_parser_peek_token(parser)->keyword == RID_EXTENSION);
			c_parser_declaration_or_fndef(parser, true, true, true, false, true,
					  ...
					  bool is_defn)
{
	...
	if (data->error_seen || fndecl == error_mark_node)
		return;
	...
	int compatible = oacc_verify_routine_clauses(fndecl, &data->clauses, data->loc,
												 "#pragma acc routine");
	if (compatible < 0)
	{
		data->error_seen = true;
...
#define OACC_UPDATE_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_ASYNC) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_DEVICE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_HOST) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_IF) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_IF_PRESENT) | (OMP_CLAUSE_MASK_1 << PRAGMA_OACC_CLAUSE_WAIT))
...
c_parser_omp_atomic(location_t loc, c_parser *parser)
{
	...
	while (c_parser_next_token_is_not(parser, CPP_PRAGMA_EOL))
	{
		...
		if (c_parser_next_token_is(parser, CPP_NAME))
		{
			...
			enum omp_memory_order new_memory_order = OMP_MEMORY_ORDER_UNSPECIFIED;
	...
	if (memory_order == OMP_MEMORY_ORDER_UNSPECIFIED)
	{
		omp_requires_mask = (enum omp_requires)(omp_requires_mask | OMP_REQUIRES_ATOMIC_DEFAULT_MEM_ORDER_USED);
		switch ((enum omp_memory_order)(omp_requires_mask & OMP_REQUIRES_ATOMIC_DEFAULT_MEM_ORDER))
		{
		...
		{
		case OMP_ATOMIC_READ:
			if (memory_order == OMP_MEMORY_ORDER_ACQ_REL || memory_order == OMP_MEMORY_ORDER_RELEASE)
			{
			...
			break;
		case NOP_EXPR: /* atomic write */
			if (memory_order == OMP_MEMORY_ORDER_ACQ_REL || memory_order == OMP_MEMORY_ORDER_ACQUIRE)
			{
...
restart:
	...
	case COMPOUND_EXPR:
		if (TREE_CODE(TREE_OPERAND(lhs, 0)) == SAVE_EXPR && TREE_CODE(TREE_OPERAND(lhs, 1)) == COMPOUND_EXPR && TREE_CODE(TREE_OPERAND(TREE_OPERAND(lhs, 1), 0)) == MODIFY_EXPR && TREE_OPERAND(TREE_OPERAND(lhs, 1), 1) == TREE_OPERAND(lhs, 0) && TREE_CODE(TREE_TYPE(TREE_OPERAND(TREE_OPERAND(TREE_OPERAND(lhs, 1), 0), 0))) == BOOLEAN_TYPE)
			/* Undo effects of boolean_increment for post {in,de}crement.  */
			lhs = TREE_OPERAND(TREE_OPERAND(lhs, 1), 0);
		/* FALLTHRU */
	case MODIFY_EXPR:
		if (TREE_CODE(lhs) == MODIFY_EXPR && TREE_CODE(TREE_TYPE(TREE_OPERAND(lhs, 0))) == BOOLEAN_TYPE)
		{
			...
			if (integer_onep(TREE_OPERAND(lhs, 1)))
			{
				...
				unfolded_lhs = NULL_TREE;
				opcode = NOP_EXPR;
				if (code == OMP_ATOMIC_CAPTURE_NEW && !structured_block && TREE_CODE(orig_lhs) == COMPOUND_EXPR)
					code = OMP_ATOMIC_CAPTURE_OLD;
				break;
			}
			if (TREE_CODE(TREE_OPERAND(lhs, 1)) == TRUTH_NOT_EXPR && TREE_OPERAND(lhs, 0) == TREE_OPERAND(TREE_OPERAND(lhs, 1), 0))
			{
...
stmt_done:
	...
	if (unfolded_lhs && unfolded_lhs1 && !c_tree_equal(unfolded_lhs, unfolded_lhs1))
	{
...
c_parser_omp_critical(location_t loc, c_parser *parser, bool *if_p)
{
	...
	if (c_parser_next_token_is(parser, CPP_OPEN_PAREN))
	{
		...
		if (c_parser_next_token_is(parser, CPP_COMMA) && c_parser_peek_2nd_token(parser)->type == CPP_NAME)
			c_parser_consume_token(parser);
	...
	if (c_parser_next_token_is(parser, CPP_NAME))
	{
		...
		else if (!strcmp("update", p))
		{
			...
			if (c_parens.require_open(parser))
			{
				...
				if (c_parser_next_token_is(parser, CPP_NAME))
				{
					const char *p2 = IDENTIFIER_POINTER(c_parser_peek_token(parser)->value);
...
c_parser_omp_scan_loop_body(c_parser *parser, bool open_brace_parsed)
{
	...
	loc = c_parser_peek_token(parser)->location;
	if (!open_brace_parsed && !c_parser_require(parser, CPP_OPEN_BRACE, "expected %<{%>"))
	{
					  ...
					  tree clauses, tree *cclauses, bool *if_p)
{
	...
	for (cl = clauses; cl; cl = OMP_CLAUSE_CHAIN(cl))
		...
		else if (OMP_CLAUSE_CODE(cl) == OMP_CLAUSE_ORDERED && OMP_CLAUSE_ORDERED_EXPR(cl))
		{
			ordered_cl = cl;
			ordered = tree_to_shwi(OMP_CLAUSE_ORDERED_EXPR(cl));
		}
		else if (OMP_CLAUSE_CODE(cl) == OMP_CLAUSE_REDUCTION && OMP_CLAUSE_REDUCTION_INSCAN(cl) && (code == OMP_SIMD || code == OMP_FOR))
			inscan = true;
	...
	if (ordered && ordered < collapse)
	{
		...
		OMP_CLAUSE_ORDERED_EXPR(ordered_cl) = build_int_cst(NULL_TREE, collapse);
		ordered = collapse;
	}
	if (ordered)
	{
	...
	for (i = 0; i < count; i++)
	{
		...
		else if (c_parser_next_token_is(parser, CPP_NAME) && c_parser_peek_2nd_token(parser)->type == CPP_EQ)
		{
		...
		{
			location_t cond_loc = c_parser_peek_token(parser)->location;
			struct c_expr cond_expr = c_parser_binary_expression(parser, NULL, NULL_TREE);
	...
	parse_next:
	 ...
	 For now handle (multiple) {'s and empty statements.  */
		do
		{
			...
			else if (bracecount && c_parser_next_token_is(parser, CPP_SEMICOLON))
				c_parser_consume_token(parser);
			else
			{
	...
	if (!fail)
	{
		...
		if (stmt)
		{
			...
			if (cclauses != NULL && cclauses[C_OMP_CLAUSE_SPLIT_PARALLEL] != NULL)
			{
				tree *c;
				for (c = &cclauses[C_OMP_CLAUSE_SPLIT_PARALLEL]; *c;)
					if (OMP_CLAUSE_CODE(*c) != OMP_CLAUSE_FIRSTPRIVATE && OMP_CLAUSE_CODE(*c) != OMP_CLAUSE_LASTPRIVATE)
...
#define OMP_LOOP_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_PRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_LASTPRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_REDUCTION) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_COLLAPSE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_BIND) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_ORDER))
...
#define OMP_SIMD_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_SAFELEN) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_SIMDLEN) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_LINEAR) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_ALIGNED) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_PRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_LASTPRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_REDUCTION) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_COLLAPSE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_IF) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_NONTEMPORAL) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_ORDER))
...
#define OMP_ORDERED_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_THREADS) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_SIMD))
...
#define OMP_SECTIONS_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_PRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_FIRSTPRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_LASTPRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_REDUCTION) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_NOWAIT))
					  ...
					  bool *if_p)
{
	...
	if ((mask & (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_MAP)) != 0 && (mask & (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_DIST_SCHEDULE)) == 0)
	...
	if (c_parser_next_token_is_keyword(parser, RID_FOR))
	{
		...
		stmt = c_finish_omp_parallel(loc, cclauses[C_OMP_CLAUSE_SPLIT_PARALLEL],
									 block);
		if (ret == NULL_TREE)
			return ret;
	...
	else if (cclauses && (mask & (OMP_CLAUSE_MASK_1
...
#define OMP_SINGLE_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_PRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_FIRSTPRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_COPYPRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_NOWAIT))
...
c_parser_omp_single(location_t loc, c_parser *parser, bool *if_p)
{
	...
	OMP_SINGLE_CLAUSES(stmt) = c_parser_omp_all_clauses(parser, OMP_SINGLE_CLAUSE_MASK,
														"#pragma omp single");
...
c_parser_omp_taskwait(c_parser *parser)
{
	...
	tree clauses = c_parser_omp_all_clauses(parser, OMP_TASKWAIT_CLAUSE_MASK,
											"#pragma omp taskwait");
...
#define OMP_CANCEL_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_PARALLEL) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_FOR) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_SECTIONS) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_TASKGROUP) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_IF))
...
#define OMP_CANCELLATION_POINT_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_PARALLEL) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_FOR) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_SECTIONS) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_TASKGROUP))
...
c_parser_omp_cancellation_point(c_parser *parser, enum pragma_context context)
{
	...
	clauses = c_parser_omp_all_clauses(parser, OMP_CANCELLATION_POINT_CLAUSE_MASK,
...
#define OMP_DISTRIBUTE_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_PRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_FIRSTPRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_LASTPRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_DIST_SCHEDULE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_COLLAPSE))
...
#define OMP_TEAMS_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_PRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_FIRSTPRIVATE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_SHARED) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_REDUCTION) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_NUM_TEAMS) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_THREAD_LIMIT) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_DEFAULT))
...
#define OMP_TARGET_DATA_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_DEVICE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_MAP) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_IF) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_USE_DEVICE_PTR) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_USE_DEVICE_ADDR))

static tree
c_parser_omp_target_data(location_t loc, c_parser *parser, bool *if_p)
{
	tree clauses = c_parser_omp_all_clauses(parser, OMP_TARGET_DATA_CLAUSE_MASK,
	...
	for (tree *pc = &clauses; *pc;)
	{
		...
		else if (OMP_CLAUSE_CODE(*pc) == OMP_CLAUSE_USE_DEVICE_PTR || OMP_CLAUSE_CODE(*pc) == OMP_CLAUSE_USE_DEVICE_ADDR)
			map_seen = 3;
		pc = &OMP_CLAUSE_CHAIN(*pc);
	}
...
#define OMP_TARGET_UPDATE_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_FROM) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_TO) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_DEVICE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_IF) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_DEPEND) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_NOWAIT))
						   ...
						   enum pragma_context context)
{
	...
	tree clauses = c_parser_omp_all_clauses(parser, OMP_TARGET_UPDATE_CLAUSE_MASK,
											"#pragma omp target update");
	if (omp_find_clause(clauses, OMP_CLAUSE_TO) == NULL_TREE && omp_find_clause(clauses, OMP_CLAUSE_FROM) == NULL_TREE)
	{
		error_at(loc,
...
#define OMP_TARGET_ENTER_DATA_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_DEVICE) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_MAP) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_IF) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_DEPEND) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_NOWAIT))
							   ...
							   enum pragma_context context)
{
	...
	tree clauses = c_parser_omp_all_clauses(parser, OMP_TARGET_ENTER_DATA_CLAUSE_MASK,
							  ...
							  enum pragma_context context)
{
	...
	tree clauses = c_parser_omp_all_clauses(parser, OMP_TARGET_EXIT_DATA_CLAUSE_MASK,
...
c_parser_omp_target(c_parser *parser, enum pragma_context context, bool *if_p)
{
	...
	if (flag_openmp)
		omp_requires_mask = (enum omp_requires)(omp_requires_mask | OMP_REQUIRES_TARGET_USED);
		...
		if (ccode != ERROR_MARK)
		{
		 ...
		 before entering the target construct.  */
				...
				for (c = cclauses[C_OMP_CLAUSE_SPLIT_TEAMS];
					...
					if ((OMP_CLAUSE_CODE(c) == OMP_CLAUSE_NUM_TEAMS || OMP_CLAUSE_CODE(c) == OMP_CLAUSE_THREAD_LIMIT) && TREE_CODE(OMP_CLAUSE_OPERAND(c, 0)) != INTEGER_CST)
					{
						...
						OMP_CLAUSE_CHAIN(tc) = cclauses[C_OMP_CLAUSE_SPLIT_TARGET];
	...
	OMP_TARGET_CLAUSES(stmt) = c_parser_omp_all_clauses(parser, OMP_TARGET_CLAUSE_MASK,
														"#pragma omp target");
	pc = &OMP_TARGET_CLAUSES(stmt);
	keep_next_level();
	block = c_begin_compound_stmt(true);
...
#define OMP_DECLARE_SIMD_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_SIMDLEN) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_LINEAR) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_ALIGNED) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_UNIFORM) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_INBRANCH) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_NOTINBRANCH))
...
c_parser_omp_declare_simd(c_parser *parser, enum pragma_context context)
{
	...
	gcc_assert(strcmp(IDENTIFIER_POINTER(kind), "simd") == 0 || strcmp(IDENTIFIER_POINTER(kind), "variant") == 0);
	...
	while (c_parser_next_token_is(parser, CPP_PRAGMA))
	{
		if (c_parser_peek_token(parser)->pragma_kind != PRAGMA_OMP_DECLARE || c_parser_peek_2nd_token(parser)->type != CPP_NAME || c_parser_peek_2nd_token(parser)->value != kind)
		{
...
c_parser_omp_context_selector(c_parser *parser, tree set, tree parms)
{
	...
	{
		tree selector;
		if (c_parser_next_token_is(parser, CPP_KEYWORD) || c_parser_next_token_is(parser, CPP_NAME))
		...
		if (property_kind == CTX_PROPERTY_NAME_LIST && IDENTIFIER_POINTER(set)[0] == 'i' && strcmp(IDENTIFIER_POINTER(selector), "atomic_default_mem_order") == 0)
		...
		if (c_parser_next_token_is(parser, CPP_OPEN_PAREN))
		{
			...
			c_token *token = c_parser_peek_token(parser);
			if (allow_score && c_parser_next_token_is(parser, CPP_NAME) && strcmp(IDENTIFIER_POINTER(token->value), "score") == 0 && c_parser_peek_2nd_token(parser)->type == CPP_OPEN_PAREN)
			{
				c_parser_consume_token(parser);
				...
				if (score != error_mark_node)
				{
					...
					if (!INTEGRAL_TYPE_P(TREE_TYPE(score)) || TREE_CODE(score) != INTEGER_CST)
			...
			case CTX_PROPERTY_USER:
				do
				{
					...
					else if (t != error_mark_node)
					{
						mark_exp_read(t);
						t = c_fully_fold(t, false, NULL);
						if (!INTEGRAL_TYPE_P(TREE_TYPE(t)) || !tree_fits_shwi_p(t))
			...
			case CTX_PROPERTY_SIMD:
				...
				c = c_parser_omp_all_clauses(parser,
											 ...
											 "simd", true, 2);
				c = c_omp_declare_simd_clauses_to_numbers(parms == error_mark_node
															  ? NULL_TREE
															  : parms,
														  c);
				properties = c;
		...
		else if (property_kind == CTX_PROPERTY_NAME_LIST || property_kind == CTX_PROPERTY_ID || property_kind == CTX_PROPERTY_EXPR)
		{
...
c_finish_omp_declare_variant(c_parser *parser, tree fndecl, tree parms)
{
	...
	if (ctx != error_mark_node && variant != error_mark_node)
	{
		...
		else if (omp_get_context_selector(ctx, "construct", "simd") == NULL_TREE && !comptypes(TREE_TYPE(fndecl), TREE_TYPE(variant)))
		{
		...
		else if (fndecl_built_in_p(variant) && (strncmp(IDENTIFIER_POINTER(DECL_NAME(variant)),
						  ...
						  vec<c_token> clauses)
{
	...
	if (clauses.exists() && (clauses[0].type == CPP_NAME || clauses[0].type == CPP_PRAGMA))
	...
	{
		c_token *token = c_parser_peek_token(parser);
		gcc_assert(token->type == CPP_NAME && strcmp(IDENTIFIER_POINTER(token->value), kind) == 0);
		c_parser_consume_token(parser);
		parser->in_pragma = true;
...
#define OMP_DECLARE_TARGET_CLAUSE_MASK \
	((OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_TO) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_LINK) | (OMP_CLAUSE_MASK_1 << PRAGMA_OMP_CLAUSE_DEVICE_TYPE))
...
c_parser_omp_end_declare_target(c_parser *parser)
{
	location_t loc = c_parser_peek_token(parser)->location;
	c_parser_consume_pragma(parser);
	if (c_parser_next_token_is(parser, CPP_NAME) && strcmp(IDENTIFIER_POINTER(c_parser_peek_token(parser)->value),
														   "declare") == 0)
	{
		c_parser_consume_token(parser);
		if (c_parser_next_token_is(parser, CPP_NAME) && strcmp(IDENTIFIER_POINTER(c_parser_peek_token(parser)->value),
...
c_parser_omp_declare_reduction(c_parser *parser, enum pragma_context context)
{
	...
	while (true)
	{
		...
		if (ctype != NULL)
		{
			...
			if (type == error_mark_node)
				;
			else if ((INTEGRAL_TYPE_P(type) || TREE_CODE(type) == REAL_TYPE || TREE_CODE(type) == COMPLEX_TYPE) && orig_reduc_id == NULL_TREE)
				error_at(loc, "predeclared arithmetic type in "
							  "%<#pragma omp declare reduction%>");
			else if (TREE_CODE(type) == FUNCTION_TYPE || TREE_CODE(type) == ARRAY_TYPE)
				...
				for (t = DECL_INITIAL(reduc_decl); t; t = TREE_CHAIN(t))
					if (comptypes(TREE_PURPOSE(t), type))
					{
						error_at(loc, "redeclaration of %qs "
									  ...
									  "type %qT",
								 IDENTIFIER_POINTER(reduc_id) + sizeof("omp declare reduction ") - 1,
								 type);
						location_t ploc = DECL_SOURCE_LOCATION(TREE_VEC_ELT(TREE_VALUE(t),
																			0));
	...
	FOR_EACH_VEC_ELT(types, i, type)
	{
	 ...
	 warn about these.  */
		...
		if (!c_parser_require(parser, CPP_CLOSE_PAREN, "expected %<)%>"))
			bad = true;
		else if (c_parser_next_token_is(parser, CPP_NAME) && strcmp(IDENTIFIER_POINTER(c_parser_peek_token(parser)->value),
																	"initializer") == 0)
		{
			c_parser_consume_token(parser);
			pop_scope();
			...
			else if (strcmp(IDENTIFIER_POINTER(c_parser_peek_token(parser)->value),
							"omp_priv") != 0)
			{
				if (c_parser_peek_2nd_token(parser)->type != CPP_OPEN_PAREN || c_parser_peek_token(parser)->id_kind != C_ID_ID)
				{
					c_parser_error(parser, "expected function-name %<(%>");
					bad = true;
				}
		...
		if (c_parser_next_token_is(parser, CPP_NAME))
		{
			...
			else if (!strcmp(p, "atomic_default_mem_order"))
			{
				...
				if (parens.require_open(parser))
				{
					if (c_parser_next_token_is(parser, CPP_NAME))
					{
						...
						if (!strcmp(p, "seq_cst"))
							this_req = (enum omp_requires)OMP_MEMORY_ORDER_SEQ_CST;
						else if (!strcmp(p, "relaxed"))
							this_req = (enum omp_requires)OMP_MEMORY_ORDER_RELAXED;
						else if (!strcmp(p, "acq_rel"))
							this_req = (enum omp_requires)OMP_MEMORY_ORDER_ACQ_REL;
					}
					if (this_req == 0)
					{
						error_at(c_parser_peek_token(parser)->location,
								 ...
								 "%<acq_rel%>");
						if (c_parser_peek_2nd_token(parser)->type == CPP_CLOSE_PAREN)
							c_parser_consume_token(parser);
					}
					else
						c_parser_consume_token(parser);
			...
			if (this_req)
			{
				if ((this_req & ~OMP_REQUIRES_ATOMIC_DEFAULT_MEM_ORDER) != 0)
				{
					...
					if (this_req != OMP_REQUIRES_DYNAMIC_ALLOCATORS && (omp_requires_mask & OMP_REQUIRES_TARGET_USED) != 0)
						error_at(cloc, "%qs clause used lexically after first "
									   "target construct or offloading API",
								 p);
				}
				...
				else if ((omp_requires_mask & OMP_REQUIRES_ATOMIC_DEFAULT_MEM_ORDER) != 0)
				{
					error_at(cloc, "more than one %<atomic_default_mem_order%>"
								   " clause in a single compilation unit");
					this_req = (enum omp_requires)(omp_requires_mask & OMP_REQUIRES_ATOMIC_DEFAULT_MEM_ORDER);
				}
				else if ((omp_requires_mask & OMP_REQUIRES_ATOMIC_DEFAULT_MEM_ORDER_USED) != 0)
				...
				omp_requires_mask = (enum omp_requires)(omp_requires_mask | this_req);
				continue;
			}
		}
		break;
					  ...
					  bool *if_p)
{
	...
	if (c_parser_next_token_is(parser, CPP_NAME))
	{
		...
		if (strcmp(p, "simd") == 0)
		{
			...
			OMP_FOR_CLAUSES(ret) = c_finish_taskloop_clauses(OMP_FOR_CLAUSES(ret));
			SET_EXPR_LOCATION(ret, loc);
			add_stmt(ret);
			return ret;
		}
...
c_parser_transaction(c_parser *parser, enum rid keyword)
{
	...
	gcc_assert((keyword == RID_TRANSACTION_ATOMIC || keyword == RID_TRANSACTION_RELAXED) && c_parser_next_token_is_keyword(parser, keyword));
	c_parser_consume_token(parser);
	...
	if (flag_tm)
		...
		error_at(loc, (keyword == RID_TRANSACTION_ATOMIC ? "%<__transaction_atomic%> without transactional memory support enabled"
